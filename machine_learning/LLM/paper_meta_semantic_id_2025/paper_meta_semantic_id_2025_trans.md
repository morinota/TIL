...
Token parameterization techniques 
トークンのパラメータ化技術
...
特定の特徴、例えばターゲットアイテムに対して、モデルが考慮する異なるアイテムの数 $I$ は、スパースモジュールにおける実行可能な埋め込みテーブルサイズ $H$ よりもはるかに大きくなる可能性があります。
...
Impression Skew – cumulative impressions as a function of the share of items considered. 
インプレッションの偏り – 考慮されたアイテムの割合に対する累積インプレッション。

As items are sorted by the impression count, one sees that the majority of impressions comes from a fraction of most popular items. 
アイテムがインプレッション数でソートされると、ほとんどのインプレッションが最も人気のあるアイテムの一部から来ていることがわかります。
...
テイルアイテムはトレーニング例が少ないため、一般化の良い埋め込み表現 $\mathbf{e}(\mathbf{x})$ を学習することは困難です。
...
既存のアイテムID空間は非常に動的で、多くの古いアイテムが退役し（図3）、新しいアイテムがシステムに入ってきます。 
We call this item distribution shift in our system “raw ID drifting.” 
このアイテム分布の変化を私たちのシステムでは「生のID漂流（raw ID drifting）」と呼びます。 
...
生のID漂流現象は、オンライン推薦システムの特性に起因しており、新しい広告が日々作成され、ほとんどの広告は比較的短い寿命を持っています。 
...
私たちは、生のIDからセマンティックIDに切り替えることで、上記の問題に効果的に対処できると仮定します。
...
同様に、テールアイテム $\mathbf{x}$ がヘッドまたはトルソーアイテム $\mathbf{y}$ と類似のコンテンツを持つ場合、彼らのセマンティックIDは一致するか（少なくとも接頭辞を共有します）。
...
これは、モデルが多くのトレーニング例を持つアイテム $\mathbf{y}$ からアイテム $\mathbf{x}$ へ知識を転送する方法です。
...
要約すると、セマンティック概念の時間的安定性は、セマンティックIDエンコーディングの安定性をもたらし、モデルの埋め込み表現の不安定性を軽減します。

Figure 3:  
図3:

ID Drift – share of items that remain active in the initial corpus as a function of time.  
IDドリフト - 初期コーパス内でアクティブなアイテムの割合を時間の関数として示します。

Half of the original corpus exits the system after 6 days.  
元のコーパスの半分は6日後にシステムから退出します。

An equal number of new items enters the system, creating a severe item distribution drift.  
同数の新しいアイテムがシステムに入ることで、深刻なアイテム分布のドリフトが生じます。

Table 3:  
表3:

Performance of three item representation approaches over various item segments.  
さまざまなアイテムセグメントにおける3つのアイテム表現アプローチの性能。

(a)  
(a)

Evaluation NE (lower is better). Semantic ID enables knowledge transfer to tail and new cold start items.  
評価NE（低いほど良い）。セマンティックIDは、テールおよび新しいコールドスタートアイテムへの知識転送を可能にします。
...
$\textbf{NE}[t_{0},t_{1}] - \textbf{NE}[t_{0}+42h,t_{1}+42h]$.  
$\textbf{NE}[t_{0},t_{1}] - \textbf{NE}[t_{0}+42h,t_{1}+42h]$。
...
$I \approx a \cdot H$のとき、$a > 1$のために、私たちは生のIDを埋め込みテーブルの行にランダムにハッシュすることができます。
...
アイテムのコンテンツ埋め込みは、マルチモーダルな画像とテキストの基盤モデルから取得されます。
...
アイテム埋め込みテーブルのサイズは、IEのアイテムの総数と等しく、RHおよびSemIDのために小さいサイズに設定され、平均衝突係数は3です。

The user history features are mapped using random hashing. 
ユーザ履歴の特徴は、ランダムハッシングを使用してマッピングされます。
...
前回と同様に、ソートされた順序で累積インプレッション数の25%、75%、または100%を生成するかどうかに応じて、アイテムをヘッド、トルソー、テールアイテムにセグメント化します。
...
トレーニング中に見られなかった評価期間中にのみ現れる新しいアイテムのセグメントでも評価を行います。
...
ベースラインと比較して、Semantic IDはテールアイテムの一般化を改善し、ヘッドアイテムにはNE中立で、トルソーアイテムにはわずかに有益です。
...
これは個別の埋め込みアプローチに対しても相対的であり、Semantic IDはランダムハッシングよりも単にクラスタリングが優れているわけではなく、ターゲットアイテムの特徴が意味に基づく知識共有から利益を得ていることがわかります。
...
知識共有の利益は新しいアイテムセグメントで最大であり、SemIDはRHおよびIEに対して大きな利益を達成します（それぞれ−0.41%および−0.33%）。
...
新しいアイテムは、トレーニング中に見られた意味的に類似したアイテムからの事前トレーニングされた重みを予測に使用し、無関係な重み（RH）や未トレーニングの重み（IE）を使用するのではありません。
...
個別の埋め込みアプローチは、ランダムハッシングと比較してパフォーマンスのギャップが小さいです。
...
対照的に、Semantic IDは個別の埋め込みのパフォーマンスに匹敵し、その学習された表現が時間とともにより安定していることを示しています。
...
この改善された表現の安定性が、IDの漂流がさらに顕著になる長期間のトレーニングにおいてモデルがより良く一般化できることを可能にすると推測します。
...
RHおよびSemIDモデルを20日間トレーニングし、期間の最後の4日間のみトレーニングされた対応するモデルと比較します。
...
NE improvement from training for 20 days of data instead of 4 days. 
4日間ではなく20日間のデータでトレーニングした際のNEの改善。
...
私たちは、Semantic IDによって生成された意味に基づく分割が、ランダムハッシングによって生成されたランダムな分割よりも推薦問題に適しているかどうかを確認したいと考えています。
...
より低い内部パーティション埋め込み分散とより高い外部パーティション距離を持つパーティションは、個々の埋め込みのより効果的な要約と見なすことができます。
...
Intra- and inter-cluster variances and pairwise distances for random hashing and SemID-based partitions. 
ランダムハッシングおよびSemIDベースのパーティションのための内部および外部クラスターの分散とペアワイズ距離。
...
私たちは、Semantic IDパーティションがランダムハッシングと比較して、より低い内部クラスター分散を持つクラスターを生成することを観察します。
...
このセクションでは、Semantic IDがユーザ履歴モデリングに与える影響を探ります。
...
私たちは、Semantic IDと文脈化された注意ベースの集約モジュール（PMAまたはTransformer）を使用することで、シーケンスを文脈化しないベースライン（Bypass）と比較して大きな利益が得られることを発見しました。
...
Performance for three aggregation modules. 
3つの集約モジュールの性能。

Baseline: model with RH for each module. 
ベースライン：各モジュールに対するRHを持つモデル。

Semantic ID brings larger gains to the contextualizing modules. 
Semantic IDは文脈化モジュールに対してより大きな利益をもたらします。
...
Attention score-based evaluation metrics for random hashing and SemID-based models for the user history item interaction features. 
ユーザ履歴アイテム相互作用機能に対するランダムハッシングおよびSemIDベースのモデルの注意スコアに基づく評価メトリック。
...
これらのメトリックは、Semantic IDアイテム表現がユーザ履歴モデリングにおいてランダムハッシングベースの対応物よりもより安定しており、意味があることを示す有望な信号です。
...
Semantic ID機能は、Meta Ads Recommendation Systemで1年以上にわたり生産化されています。
...
RQ-VAEモデルは、Metaの広告ランキングのためのコンテンツ理解（CU）モデルでトレーニングされます。

The CU models are pre-trained on the public CC100 dataset Conneau (2020) and then fine-tuned on internal ads datasets. 
CUモデルは、公開されたCC100データセット（Conneau, 2020）で事前トレーニングされ、その後、内部広告データセットでファインチューニングされます。
...
Semantic ID serving pipeline. セマンティックIDサービングパイプライン。
...
特徴生成段階では、ターゲットアイテムの生IDとユーザーエンゲージメントの生ID履歴が、エンティティデータストアからのセマンティックID信号で強化され、セマンティック特徴が生成されます。

When serving requests arrive, the precomputed features are fetched and passed to the downstream ranking models. 
リクエストが到着すると、事前計算された特徴が取得され、下流のランキングモデルに渡されます。
...
Meta広告ランキングでは、オフラインのNEの向上が0.02%を超えると重要と見なされます。
...
Meta Adsレコメンダーは数十億のユーザーにサービスを提供し、会社の中で最も最適化されたモデルの1つであるため、0.15%のオンライン性能向上は重要と見なされます。

Table 8: 
表8：

NE improvement from incorporating Semantic ID features in the flagship Meta ads ranking model. 
フラッグシップMeta広告ランキングモデルにおけるSemantic ID特徴を組み込むことによるNEの改善。
...
Baseline + 6 sparse features 
ベースライン + 6つのスパース特徴

Baseline + 1 sequential feature 
ベースライン + 1つのシーケンシャル特徴



### 7.4 Semantic and Prediction Similarity 7.4 セマンティックおよび予測類似性

Intuitively, one may think that if two items are semantically similar, their user engagement patterns will also be similar. 
直感的には、2つのアイテムがセマンティックに類似している場合、それらのユーザーエンゲージメントパターンも類似していると考えるかもしれません。しかし、ユーザーの行動や認識はより微妙であり、セマンティクスに関して予測可能に連続しているわけではありません。
...
この相関を測定するために、私たちはオンラインA/Bテストを実施し、システムによってユーザーに推奨されるアイテムのセット$S$を選択します。
...
Click Loss Rate reduction from Semantic ID. 
Semantic IDからのクリック損失率の減少。
...
Semantic IDがアイテムのセマンティクスに基づいてアイテムコーパスを分割するため、予測類似性はセマンティック類似性と相関していると結論付けます。
...
reduction in average AAR 平均AARの削減

Section 6.2 セクション6.2
...
- (1)↑
- Chang etal.(2024)↑Bo Chang, Changping Meng, He Ma, Shuo Chang, Yang Gu, Yajun Peng, Jingchen Feng, Yaping Zhang, Shuchao Bi, EdH Chi, etal.2024.Cluster Anchor Regularization to Alleviate Popularity Bias in Recommender Systems. InCompanion Proceedings of the ACM on Web Conference 2024. 151–160.
- Chang et al. (2024)↑Bo Chang, Changping Meng, He Ma, Shuo Chang, Yang Gu, Yajun Peng, Jingchen Feng, Yaping Zhang, Shuchao Bi, EdH Chi, 2024年。推薦システムにおける人気バイアスを緩和するためのクラスタアンカー正則化。ACM Web Conference 2024の付録論文集に掲載。151–160ページ。
- Conneau (2020)↑Alexis Conneau. 2019.Unsupervised cross-lingual representation learning at scale.arXiv preprint arXiv:1911.02116(2019).
- Conneau (2020)↑アレクシス・コネー。2019年。大規模な教師なしクロスリンガル表現学習。arXivプレプリント arXiv:1911.02116（2019）。
- Covington etal.(2016)↑Paul Covington, Jay Adams, and Emre Sargin. 2016.Deep neural networks for youtube recommendations. InProceedings of the 10th ACM conference on recommender systems. 191–198.
- Covington et al. (2016)↑ポール・コバントン、ジェイ・アダムス、エムレ・サルギン。2016年。YouTubeの推薦のための深層ニューラルネットワーク。第10回ACM推薦システム会議の論文集に掲載。191–198ページ。
...
- Devlin (2018)↑ジェイコブ・デブリン。2018年。BERT: 言語理解のための深層双方向トランスフォーマーの事前学習。arXivプレプリント arXiv:1810.04805（2018）。
- Gama etal.(2014)↑João Gama, Indrė Žliobaitė, Albert Bifet, Mykola Pechenizkiy, and Abdelhamid Bouchachia. 2014.A survey on concept drift adaptation.ACM computing surveys (CSUR)46, 4 (2014), 1–37.
- Gama et al. (2014)↑ジョアン・ガマ、インドレ・ズリオバイテ、アルバート・ビフェット、ミコラ・ペチェニツキー、アブデルハミド・ブチャチア。2014年。概念ドリフト適応に関する調査。ACMコンピューティングサーベイ（CSUR）46巻、4号（2014年）、1–37ページ。
- Hou etal.(2023)↑Yupeng Hou, Zhankui He, Julian McAuley, and WayneXin Zhao. 2023.Learning vector-quantized item representation for transferable sequential recommenders. InProceedings of the ACM Web Conference 2023. 1162–1171.
- Hou et al. (2023)↑ユーペン・ホウ、ジャンクイ・ハ、ジュリアン・マカウリー、ウェイン・シン・ジャオ。2023年。移植可能な逐次推薦者のためのベクトル量子化されたアイテム表現の学習。ACM Web Conference 2023の論文集に掲載。1162–1171ページ。
- Kudo (2018)↑Taku Kudo. 2018.SentencePiece: A simple and language independent subword tokenizer and detokenizer for neural text processing.arXiv preprint arXiv:1808.06226(2018).
- Kudo (2018)↑工藤拓。2018年。SentencePiece: ニューラルテキスト処理のためのシンプルで言語に依存しないサブワードトークナイザーおよびデトークナイザー。arXivプレプリント arXiv:1808.06226（2018）。
- Lee etal.(2019)↑Juho Lee, Yoonho Lee, Jungtaek Kim, Adam Kosiorek, Seungjin Choi, and YeeWhye Teh. 2019.Set transformer: A framework for attention-based permutation-invariant neural networks. InInternational conference on machine learning. PMLR, 3744–3753.
- Lee et al. (2019)↑ジュホ・リー、ユンホ・リー、ジョンテク・キム、アダム・コシオレク、スンジン・チョイ、イー・ホワ・テ。2019年。セットトランスフォーマー: 注意に基づく順列不変ニューラルネットワークのためのフレームワーク。国際機械学習会議において。PMLR、3744–3753ページ。
- Liu etal.(2022)↑Zhuoran Liu, Leqi Zou, Xuan Zou, Caihua Wang, Biao Zhang, Da Tang, Bolin Zhu, Yijie Zhu, Peng Wu, Ke Wang, and Youlong Cheng. 2022.Monolith: Real Time Recommendation System With Collisionless Embedding Table.arXiv:2209.07663[cs.IR]https://arxiv.org/abs/2209.07663
- Liu et al. (2022)↑ズオラン・リウ、レキ・ゾウ、シュアン・ゾウ、カイファ・ワン、ビャオ・ジャン、ダ・タン、ボリン・ジュ、イージエ・ジュ、ペン・ウー、ケ・ワン、ユー・ロン・チェン。2022年。モノリス: 衝突のない埋め込みテーブルを用いたリアルタイム推薦システム。arXiv:2209.07663[cs.IR] https://arxiv.org/abs/2209.07663
- Milojević (2010)↑Staša Milojević. 2010.Power law distributions in information science: Making the case for logarithmic binning.Journal of the American Society for Information Science and Technology61, 12 (2010), 2417–2425.
- Milojević (2010)↑スタシャ・ミロイェビッチ。2010年。情報科学におけるパワー法則分布: 対数ビン分けの必要性を主張する。アメリカ情報科学技術協会誌61巻、12号（2010年）、2417–2425ページ。
- Naumov (2019)↑Maxim Naumov. 2019.On the dimensionality of embeddings for sparse features and data.arXiv preprint arXiv:1901.02103(2019).
- Naumov (2019)↑マキシム・ナウモフ。2019年。スパース特徴とデータの埋め込みの次元について。arXivプレプリント arXiv:1901.02103（2019）。
- Naumov etal.(2019)↑Maxim Naumov, Dheevatsa Mudigere, Hao-JunMichael Shi, Jianyu Huang, Narayanan Sundaraman, Jongsoo Park, Xiaodong Wang, Udit Gupta, Carole-Jean Wu, AlissonG Azzolini, etal.2019.Deep learning recommendation model for personalization and recommendation systems.arXiv preprint arXiv:1906.00091(2019).
- Naumov et al. (2019)↑マキシム・ナウモフ、ディーヴァッツァ・ムディゲレ、ハオ・ジュン・マイケル・シー、ジアンユ・ファン、ナラヤナン・スンダラマン、ジョンスー・パーク、シャオドン・ワン、ウディット・グプタ、キャロル・ジャン・ウー、アリソン・G・アッゾリーニ、他。2019年。パーソナライズと推薦システムのための深層学習推薦モデル。arXivプレプリント arXiv:1906.00091（2019）。
- Qu etal.(2024)↑Haohao Qu, Wenqi Fan, Zihuai Zhao, and Qing Li. 2024.TokenRec: Learning to Tokenize ID for LLM-based Generative Recommendation.arXiv preprint arXiv:2406.10450(2024).
- Qu et al. (2024)↑ハオハオ・ク、ウェンチー・ファン、ズーハイ・ジャオ、チン・リー。2024年。TokenRec: LLMベースの生成推薦のためのIDをトークン化する学習。arXivプレプリント arXiv:2406.10450（2024）。
- Radford etal.(2021)↑Alec Radford, JongWook Kim, Chris Hallacy, Aditya Ramesh, Gabriel Goh, Sandhini Agarwal, Girish Sastry, Amanda Askell, Pamela Mishkin, Jack Clark, etal.2021.Learning transferable visual models from natural language supervision. InInternational conference on machine learning. PMLR, 8748–8763.
- Radford et al. (2021)↑アレック・ラドフォード、ジョン・ウク・キム、クリス・ハラシー、アディティア・ラメシュ、ガブリエル・ゴー、サンディニ・アガルワル、ギリッシュ・サストリー、アマンダ・アスケル、パメラ・ミシュキン、ジャック・クラーク、他。2021年。自然言語の監督から移植可能な視覚モデルを学習する。国際機械学習会議において。PMLR、8748–8763ページ。
- Rajput etal.(2024)↑Shashank Rajput, Nikhil Mehta, Anima Singh, Raghunandan HulikalKeshavan, Trung Vu, Lukasz Heldt, Lichan Hong, Yi Tay, Vinh Tran, Jonah Samost, etal.2024.Recommender systems with generative retrieval.Advances in Neural Information Processing Systems36 (2024).
- Rajput et al. (2024)↑シャシャンク・ラジプト、ニキル・メータ、アニマ・シン、ラグナンダン・フリカール・ケシャヴァン、トゥルン・ヴ、ルカシュ・ヘルド、リチャン・ホン、イー・テイ、ヴィン・トラン、ジョナ・サモスト、他。2024年。生成的検索を用いた推薦システム。神経情報処理システムの進展36巻（2024年）。
- Sennrich (2015)↑Rico Sennrich. 2015.Neural machine translation of rare words with subword units.arXiv preprint arXiv:1508.07909(2015).
- Sennrich (2015)↑リコ・センリッチ。2015年。サブワードユニットを用いた希少語のニューラル機械翻訳。arXivプレプリント arXiv:1508.07909（2015）。
- Singh etal.(2023)↑Anima Singh, Trung Vu, Nikhil Mehta, Raghunandan Keshavan, Maheswaran Sathiamoorthy, Yilin Zheng, Lichan Hong, Lukasz Heldt, Li Wei, Devansh Tandon, etal.2023.Better Generalization with Semantic IDs: A Case Study in Ranking for Recommendations.arXiv preprint arXiv:2306.08121(2023).
- Singh et al. (2023)↑アニマ・シン、トゥルン・ヴ、ニキル・メータ、ラグナンダン・ケシャヴァン、マヘスワラン・サティアモーティ、イーリン・ジェン、リチャン・ホン、ルカシュ・ヘルド、リ・ウェイ、デヴァンシュ・タンデン、他。2023年。セマンティックIDによるより良い一般化: 推薦のためのランキングにおけるケーススタディ。arXivプレプリント arXiv:2306.08121（2023）。
- Vaswani (2017)↑Ashish Vaswani. 2017.Attention is all you need.Advances in Neural Information Processing Systems(2017).
- Vaswani (2017)↑アシシュ・ヴァスワニ。2017年。注意がすべて必要です。神経情報処理システムの進展（2017年）。
- Wang etal.(2017)↑Jingdong Wang, Ting Zhang, Nicu Sebe, HengTao Shen, etal.2017.A survey on learning to hash.IEEE transactions on pattern analysis and machine intelligence40, 4 (2017), 769–790.
- Wang et al. (2017)↑ジンドン・ワン、ティン・ザン、ニク・セベ、ヘン・タオ・シェン、他。2017年。ハッシュ学習に関する調査。IEEEパターン分析および機械知能トランザクション40巻、4号（2017年）、769–790ページ。
- Weinberger etal.(2009)↑Kilian Weinberger, Anirban Dasgupta, John Langford, Alex Smola, and Josh Attenberg. 2009.Feature hashing for large scale multitask learning. InProceedings of the 26th annual international conference on machine learning. 1113–1120.
- Weinberger et al. (2009)↑キリアン・ワインバーガー、アニルバン・ダスグプタ、ジョン・ラングフォード、アレックス・スモラ、ジョシュ・アッテンバーグ。2009年。大規模マルチタスク学習のための特徴ハッシング。第26回国際機械学習会議の論文集に掲載。1113–1120ページ。
- Yao etal.(2021)↑Tiansheng Yao, Xinyang Yi, DerekZhiyuan Cheng, Felix Yu, Ting Chen, Aditya Menon, Lichan Hong, EdH Chi, Steve Tjoa, Jieqi Kang, etal.2021.Self-supervised learning for large-scale item recommendations. InProceedings of the 30th ACM international conference on information & knowledge management. 4321–4330.
- Yao et al. (2021)↑ティエンシェン・ヤオ、シンヤン・イー、デレク・チー、フェリックス・ユー、ティン・チェン、アディティア・メノン、リチャン・ホン、エド・チ、スティーブ・トジョア、ジーキ・カン、他。2021年。大規模アイテム推薦のための自己教師あり学習。第30回ACM国際情報および知識管理会議の論文集に掲載。4321–4330ページ。
- Zeghidour etal.(2021)↑Neil Zeghidour, Alejandro Luebs, Ahmed Omran, Jan Skoglund, and Marco Tagliasacchi. 2021.Soundstream: An end-to-end neural audio codec.IEEE/ACM Transactions on Audio, Speech, and Language Processing30 (2021), 495–507.
- Zeghidour et al. (2021)↑ニール・ゼギドゥール、アレハンドロ・ルエブス、アフメド・オムラン、ヤン・スコグルンド、マルコ・タリアサッチ。2021年。Soundstream: エンドツーエンドのニューラルオーディオコーデック。IEEE/ACM音声、音声、言語処理トランザクション30巻（2021年）、495–507ページ。
- Zhang etal.(2020)↑Caojin Zhang, Yicun Liu, Yuanpu Xie, SofiaIra Ktena, Alykhan Tejani, Akshay Gupta, PranayKumar Myana, Deepak Dilipkumar, Suvadip Paul, Ikuhiro Ihara, etal.2020.Model size reduction using frequency based double hashing for recommender systems. InProceedings of the 14th ACM Conference on Recommender Systems. 521–526.
- Zhang et al. (2020)↑曹金・チャン、イー・チュン・リウ、ユアンプ・シー、ソフィア・イラ・クテナ、アリカン・テジャニ、アクシャイ・グプタ、プラナイ・クマール・ミャナ、ディーパク・ディリプクマール、スバディプ・ポール、イクヒロ・イハラ、他。2020年。推薦システムのための周波数ベースのダブルハッシングを用いたモデルサイズの削減。第14回ACM推薦システム会議の論文集に掲載。521–526ページ。
