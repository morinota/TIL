...
例えば、Instacartは「Lessons Learned: The Journey to Real-Time Machine Learning at Instacart」でバッチからオンライン予測への2段階の移行について議論しました。
...
DoorDashのMLインフラの責任者であるHien Luuは、MLプラットフォームを構築する過程で、企業は小さく始めるべきだとアドバイスし、DoorDashは高スループット、低レイテンシの予測サービスSybil（2020）から始めました。

After that, they introduced their feature platform Riviera (2021), then Fabricator (2022). 
その後、彼らは特徴プラットフォームRiviera（2021）を導入し、次にFabricator（2022）を導入しました。
...
彼らのデータサイエンティストの一人は、「ストリーミング機能に苦労するために四半期を費やすよりも、10のバッチ機能を試してみたい」と私たちに語りました。  
...
インフラエンジニアにScala APIがユーザーにとって採用の課題だと思うか尋ねると、通常の答えは「いいえ」です。「データサイエンティストはエンジニアですから、新しい言語を学ぶことができます。」
...
1. 集約のための異なるウィンドウタイプ（例：タムブリングウィンドウ、ホッピングウィンドウ、セッション化ウィンドウ）やポイントインタイム結合など、時間セマンティクスが不十分です。

2. Lack of composability. 
2. 合成性の欠如。

Feature pipelines can be complex. 
フィーチャーパイプラインは複雑になる可能性があります。

We need a language that allows building small pieces of logic independently and stitching them together, while making the pieces reusable. 
私たちは、小さな論理の部分を独立して構築し、それらを組み合わせることを可能にし、かつその部分を再利用可能にする言語が必要です。
...
ここ数年で、LinkedInとAirbnbの両方がフィーチャーAPIのためにPythonインターフェースに切り替えました。
...
2. 設計の決定：変換ロジックとフィーチャーロジックの分離

Transformation refers to the function applied on data: e.g. count of product views, mean of transaction value. 
変換とは、データに適用される関数を指します：例として、製品ビューのカウント、取引価値の平均などがあります。
...
例えば、データサイエンティストは変換の作成に集中でき、ストリーミングフィーチャーを計算するための長時間実行されるジョブの設定に助けが必要な場合、フィーチャー設定に関してデータエンジニアと協力することができます。
...
- Feature discoverability: Say, you have a new idea for a streaming feature to be used for your model.  
- 機能の発見可能性：たとえば、モデルに使用するためのストリーミング機能の新しいアイデアがあるとします。  
Because feature computation and storage are expensive, the first thing you might want to do is to see whether that feature is already being used by another model or another team, so that you can reuse it.  
機能の計算とストレージは高価であるため、最初に行いたいことは、その機能が他のモデルや他のチームによってすでに使用されているかどうかを確認し、それを再利用できるかどうかを確認することです。

- Source discoverability: You might want to see if this feature is actually feasible given your data setup.  
- ソースの発見可能性：この機能が実際にあなたのデータセットに対して実現可能かどうかを確認したいかもしれません。  
You might want to look up what sources are available.  
どのソースが利用可能かを調べたいかもしれません。

- Data governance: Talking about data discoverability, it’s important to consider that some data is sensitive and shouldn’t be accessible by everyone.  
- データガバナンス：データの発見可能性について話すと、いくつかのデータは機密性があり、誰でもアクセスできるべきではないことを考慮することが重要です。  
It’s important to:  
重要なことは以下の通りです：

  - Assign roles to users and configure which data should be accessible by which roles.  
  - ユーザーに役割を割り当て、どのデータがどの役割にアクセス可能であるべきかを設定します。  
  - Automate masking of sensitive, PII data.  
  - 機密性のある個人識別情報（PII）データのマスキングを自動化します。  
  - Propagate data policy – if a feature is derived from a sensitive column, it should also be marked as sensitive.  
  - データポリシーを伝播させること – 機能が機密列から派生している場合、それも機密としてマークされるべきです。
...
この機能が役立つかどうかわからないため、Redisなどのフィーチャーストアにデプロイしたくありません。なぜなら、高コストがかかる可能性があるからです。  
...
## Comparison of feature platforms 特徴プラットフォームの比較  
### Reference feature platforms 参考特徴プラットフォーム  
...
よく設計されたフィーチャープラットフォームは、新しいデータを使用して機械学習（ML）予測を改善する速度と、データサイエンティストがMLモデルを改善するための反復速度の両方を向上させることができます。
...
1. Feature staleness. If features are computed every hour, they are up to one hour stale.
   特徴の古さ。特徴が毎時計算される場合、最大で1時間古くなります。
2. Wasted computation. A company told me they need to compute features for 3 million users each time, which takes them 6 hours, therefore they can refresh their features once every 6 hours. However, only 5% of these users visit their site a day, which means that 95% of the computation is actually wasted.
   無駄な計算。ある企業は、毎回300万人のユーザの特徴を計算する必要があり、それには6時間かかるため、6時間ごとに特徴を更新できると教えてくれました。しかし、これらのユーザのうち、1日にサイトを訪れるのはわずか5%であり、つまり95%の計算が実際には無駄になっています。

Real-time features are usually the next type to be deployed. 
リアルタイム特徴は通常、次に展開されるタイプです。
...
特徴は予測時に計算されるため、新鮮であり、例えばミリ秒単位のオーダーです。
...
リアルタイム特徴は予測リクエストを受け取った際に計算されるため、その計算遅延はユーザ向けの遅延に直接加算されます。
Traffic growth and fluctuation can significantly affect computation latency. 
トラフィックの増加や変動は計算遅延に大きな影響を与える可能性があります。
...
バッチ特徴とは異なり、near RT特徴の値ははるかに頻繁に再計算されるため、特徴の古さは秒単位のオーダーになる可能性があります（それは「完全に」リアルタイムではなく「近い」だけです）。
