...
候補者:YouTubeの検索システムは非常に複雑です。**動画の関連性は、その視覚的コンテンツと、タイトルや説明などの動画に関連するテキストデータのみによって決まる**と仮定してもよいですか？
...
面接官:はい、1000万対の⟨動画、テキストクエリ⟩ペアがあると仮定しましょう。
...
**入力はテキストクエリで、出力はそのテキストクエリに関連する動画のリスト**です。
...
モデルのトレーニングのために、1000万対の⟨動画、テキストクエリ⟩のデータセットが与えられています。
...
**これを機械学習の目的に変換する一つの方法は、テキストクエリに対する関連性に基づいて動画をランク付けすること**です。
...
このアプローチでは、テキストクエリと動画は2つのエンコーダを使用して別々にエンコードされます。(=あ、Two Towerモデルアーキテクチャね...!:thinking:)
...
**逆インデックス(inverted index)は、テキストベースの検索コンポーネントを作成するための一般的な手法であり、データベース内で効率的な全文検索を可能にします。** 
...
例えば、**同じ単語がわずかに異なるスペルで表記される**ことがあります。「dog」、「dogs」、「DOG！」はすべて同じものを指しますが、異なる方法で綴られています。
...
一般的に、トークン化(tokenization)sには3つのタイプがあります：
...
(あ、これはMetaの論文で述べられてたランダムハッシュ的なアプローチかな？:thinking:)
...
- Lookup table ルックアップテーブル
  - Speed: ⚪︎ Quick to convert tokens to IDs
  - ID to token: ⚪︎ Easy to convert IDs to tokens using a reverse index table.
  - Memory: ✘ The table is stored in memory. A large number of tokens will result in an increase in memory required.
  - Unseen tokens: ✘ New or unseen words cannot be properly handled. 
  - Collisions: ⚪︎ No collisions issue
- Hashing ハッシング
  - Speed: × Need to compute the hash function to convert tokens to IDs.
  - ID to token: × Not possible to convert IDs back to tokens.
  - Memory: ⚪︎ The hash function is sufficient to convert any token to its ID.
  - Unseen tokens: ⚪︎ Easily handles new or unseen words by applying the hash function to any word.
  - Collisions: ✘ Collisions are a potential problem.
...
**テキストエンコーダを構築するためには、統計的手法と機械学習（ML）ベースの手法の2つの大きなカテゴリ**があります。
Let's examine each. 
それぞれを見てみましょう。
...
これらの手法は、統計(=頻度?)に基づいて文を特徴ベクトルに変換します。
...
TF-IDFはBoWと同じ文-単語行列を作成しますが、**単語の頻度に基づいて行列を正規化**します。
...
**テキストを埋め込みに変換するための一般的なMLベースのアプローチは3つ**あります。

- Embedding (lookup) layer
  - (i.e. これはentity embeddingね!:thinking:)
- Word2vec
...
これらのモデルは、**浅いニューラルネットワークアーキテクチャ**を使用し、局所的な文脈における**単語の共起を利用して単語の埋め込みを学習**します。
...
**Word2vecに基づく主なモデルは2つ**あります: Continuous Bag of Words (CBOW) [9] と Skip-gram [10]。
...
**Word2vecモデルとは異なり、文脈に応じて同じ単語に対して異なる埋め込みを生成**します。
...
私たちの場合、**BERTのようなTransformerベースのアーキテクチャをテキストエンコーダとして選択**します。
In some interviews, the interviewer may want you to dive deeper into the details of the Transformer-based model. 
**いくつかのインタビューでは、面接官がTransformerベースのモデルの詳細に深く掘り下げることを求めるかも**しれません。
...
**ビデオをエンコードするための2つのアーキテクチャオプション**があります：
...
**モデルがビデオ全体を処理するため、計算コストが高くなります**。

![Figure 4.16: Video-level model]()
...
**フレームレベルモデルを使用してビデオから埋め込みを抽出することは、次の3つのステップに分解**することで可能です：
...
- Aggregate (e.g., average) frame embeddings to generate the video embedding.  
  - **フレーム埋め込みを集約（例：平均）してビデオ埋め込みを生成**します。

![Figure 4.17: Frame-level model]()
...
**実際には、ビデオの時間的理解が重要でない多くのケースでフレームレベルモデルが好まれます。** (サンプリングと集約の方法を工夫したら、時間的理解もそこそこできたりするのかな??:thinking:) 
...
テキストエンコーダとビデオエンコーダをトレーニングするために、コントラスト学習アプローチを使用します。(まあここでも対照学習だよね〜:thinking:)
...
**平均逆順位（MRR）**。この指標は、**各検索結果における最初の関連項目の順位を平均する**ことによってモデルの品質を測定します。式は次の通りです。
(あ、MRRあまり意識してなかったけどシンプルだな...!:thinking:)
...
**この指標はrecall@kの欠点を解決**し、私たちのオフライン指標として使用できます。
...
動画完了率。検索結果に表示され、**ユーザーによって最後まで視聴される動画の数を測定する指標**です。 
...
NN検索を加速するために、私たちはChapter 2, Visual Search Systemで説明されている**近似最近傍（ANN）アルゴリズムを使用**します。

Text search. Using Elasticsearch, this component finds videos with titles and tags that overlap the text query.
テキスト検索。Elasticsearchを使用して、このコンポーネントは**テキストクエリと重複するタイトルやタグを持つビデオを見つけます**。
...
融合層。このコンポーネントは前のステップからの2つの異なる関連ビデオのリストを取り、それらを新しいビデオのリストに結合します。(2つのランキングの混ぜ合わせか...!:thinking:)
...
融合層は2つの方法で実装でき、その中で最も簡単なのは、**予測された関連スコアの加重和に基づいてビデオを再ランキングすること**です。
...
再ランキングサービス。このサービスは、**ビジネスレベルのロジックとポリシーを組み込む**ことによって、ビデオのランキングリストを修正します。(不適切なコンテンツのフィルタリングとかね:thinking:)
...
- Text queries can be divided into head, torso, and tail queries. 
  - テキストクエリは、ヘッド、トルソー、テールクエリに分けることができます。(学習データの人気バイアス的なやつね...!:thinking:)
