...
- 01Introduction and Overview  - 01イントロダクションと概要
- 02Visual Search System  - 02ビジュアル検索システム
- 03Google Street View Blurring System  - 03Googleストリートビューぼかしシステム
- 04YouTube Video Search  - 04YouTubeビデオ検索
- 05Harmful Content Detection  - 05有害コンテンツ検出
- 06Video Recommendation System  - 06ビデオ推薦システム
...
面接官: 実際には、モデルは画像のメタデータを使用します。しかし、簡単のために、メタデータには依存せず、画像のピクセルのみに基づくと仮定しましょう。
...
候補者: 検索はどれくらい速くあるべきでしょうか？プラットフォームに1000億から2000億の画像があると仮定すると、システムは類似の画像を迅速に取得できるべきです。それは合理的な仮定ですか？
...
一般的に、ランキング問題の目標は、画像、ウェブサイト、製品などのアイテムのコレクションをクエリに対する関連性に基づいてランク付けし、より関連性の高いアイテムが検索結果の上位に表示されるようにすることです。
...
この章では、表現学習と呼ばれる広く使用されているアプローチを使用します。
...
別の言い方をすると、モデルは入力画像を埋め込み空間と呼ばれるN次元空間の点にマッピングします。
...
これらの埋め込みは、類似した画像が空間内で互いに近接した埋め込みを持つように学習されます。
...
実際には、この空間はNNN次元であり、NNNは埋め込みベクトルのサイズです。
...
次に、埋め込み空間での距離を測定することによって、クエリ画像とプラットフォーム上の他の画像との間の類似度スコアを計算します。
...
収集した要件に基づいて、主なインタラクションのタイプはインプレッションとクリックです。
...
このセクションでは、優れた特徴量をエンジニアリングし、それらをモデルの入力として準備することについて説明します。 
...
前述のように、トレーニングに使用される各データポイントには、クエリ画像、クエリ画像に類似した正の画像、およびクエリ画像に類似しないn−1{n-1}n−1の負の画像が含まれています。 
...
- クエリ画像から類似画像を人工的に作成する（自己監視として知られる）
...
インタビューの場では、さまざまなオプションを提案し、それらのトレードオフについて議論することが重要です。 
...
異なる損失関数は対比トレーニングのために設計されており、面接官は通常、詳細な議論を期待していません。しかし、対比損失関数がどのように機能するかについての高レベルの理解を持つことは重要です。

We are going to briefly discuss how a simplified contrastive loss operates. 
ここでは、簡略化された対比損失がどのように機能するかを簡単に説明します。If you are interested in learning more about contrastive losses, refer to [9]. 
対比損失についてさらに学びたい場合は、[9]を参照してください。

As Figure 2.11 shows, we compute the contrastive loss in three steps. 
図2.11が示すように、対比損失は3つのステップで計算します。

Compute similarities. First, we compute the similarities between the query image and the embeddings of other images. 
...
Softmax. A softmax function is applied over the computed distances. 
...
Cross-entropy. Cross-entropy [15] measures how close the predicted probabilities are to the ground truth labels. 
...
類似度スコアは、000から555の間の整数であり、000は類似性がないことを示し、555は2つの画像が視覚的および意味的に非常に類似していることを示します。
...
検索、情報検索、および推薦システムは通常、同じオフラインメトリクスを共有することに注意してください。
...
ここで、$m$は出力リストの総数であり、$rank_i$は$i$番目の出力リストにおける最初の関連アイテムの順位を指します。
...
MRRは最初の関連アイテムのみを考慮し、リスト内の他の関連アイテムを無視するため、ランク付けされたリストの精度とランキングの質を測定しません。
...
Recall@k. This metric measures the ratio between the number of relevant items in the output list and the total number of relevant items available in the entire dataset. 
...
Recall@kは、モデルが出力リストに含められなかった関連アイテムの数を測定しますが、これは必ずしも良いメトリクスではありません。
...
Precision@k. This metric measures the proportion of relevant items among the top k items in the output list. 
...
このメトリクスは出力リストの精度を測定しますが、ランキングの質は考慮しません。
...
nDCG. This metric measures the ranking quality of an output list and shows how good the ranking is, compared to the ideal ranking. 
nDCG。このメトリクスは出力リストのランキングの質を測定し、理想的なランキングと比較してランキングがどれほど良いかを示します。
...
完璧なランキングシステムでは、DCGはIDCGに等しいことに注意してください。
...
その主な欠点は、真の関連スコアを導出することが常に可能であるとは限らないことです。
...
Click-through rate (CTR). This metric shows how often users click on the displayed items. 
...
図2.19は、予測パイプラインとインデックスパイプラインを示しています。
...
インデクシングサービスのもう一つの責任は、インデックステーブルを最新の状態に保つことです。
...
その効率のわずかな改善は、全体的な性能の大幅な向上につながります。
...
多くのアプリケーションでは、ユーザに十分に似たアイテムを示すことができれば十分であり、正確な最近傍検索を行う必要はありません。
...
ANNアルゴリズムは以下の3つのカテゴリに分けることができます：
...
ツリーベースのアルゴリズムは、空間を複数のパーティションに分割することによってツリーを形成します。
...
ツリー内では、非葉ノードが基準に基づいて空間を2つのパーティションに分割します。
...
アルゴリズムは、クエリポイントが属するパーティションのみを検索します。
...
LSHは特定のハッシュ関数を使用して、ポイントの次元を削減し、それらをバケットにグループ化します。
...
LSHは、クエリポイントqと同じバケットに属するポイントのみを検索します。
...
これらのアルゴリズムは、類似性に基づいてポイントをグループ化することによってクラスタを形成します。
...
クラスタが形成されると、アルゴリズムはクエリポイントが属するクラスタ内のポイントのサブセットのみを検索します。
...
これは、データポイントが限られている場合や、正確な最近傍が必要な場合に良い選択肢となります。 
...
今日のシステムで利用可能なデータの量を考えると、ANN法はより実用的な解決策です。 
...
応用機械学習の役割では、面接官がANNを実装するように求めることがあります。 
...
これらのライブラリの少なくとも1つに慣れることをお勧めします。そうすることで、概念をよりよく理解し、機械学習のコーディング面接で最近傍検索を実装する自信を得ることができます。
...
- アクティブラーニング[29]やヒューマン・イン・ザ・ループ[30]の機械学習を使用してデータをより効率的に注釈付けする方法について。
