...
我々の主な結論は、機械学習によって150の成功した製品を構築するためには、他の学問分野と統合された反復的で仮説主導のプロセスが基本であったということである。
...
チームにはそれぞれの目標があり、さまざまなビジネス指標を使用して、製品が提供する価値を定量化し、学習プロセスの中核である仮説を検証する。
Several issues make our platform a unique challenge, we briefly describe them below: High Stakes: Recommending the wrong movie, song, book, or product has relevant impact in the consumer experience.
...
ごくわずかなクエリー： 
宿泊施設を検索するユーザーは、目的地、日付、宿泊人数を指定するのがやっとです。
...
複雑な項目： 
宿泊施設の予約には、目的地、日程、宿泊施設のタイプ、部屋数、部屋のタイプ、返金ポリシーなど、いくつかの点をユーザーが決定する必要があります。
...
我々の業界に近い例では、Jannachら[5]が、レコメンダー・システムの分野が、売上の多様化、コンバージョン率、ロイヤリティといったサービス・プロバイダーに関連する指標に影響を与える指針をほとんど提供していないことを説明している。
...
本稿の残りの部分は、機械学習プロジェクトの開発プロセスの特定のフェーズ、すなわち「開始」、「モデリング」、「デプロイメント」、「モニタリング」、「評価」に関連する一連の教訓として構成されており、最後のセクションでは結論を述べている。

# Inception: Machine Learning as a Swiss Knife for Product Development 
製品開発のためのスイスナイフとしての機械学習
...
実際には、私たちのモデルは、さまざまなチームが製品を改善し、ユーザーから学ぶのを助けるツールです。
...
例えば、ユーザーインターフェースの特定の要素のサイズを最適化したり、ファネルのあるポイントや特定のコンテキストに合わせた推奨を提供したりする。
...
その反対に、意味のあるセマンティック層として機能するモデルも作成する。
...
理解しやすいコンセプトをモデル化することで、商品開発に携わるすべての人が、モデルの出力に基づいて新機能やパーソナライゼーション、説得力のあるメッセージなどを導入できるようになる。
...
例えば、ユーザーが旅行の目的地に対してどの程度フレキシブルであるかを示し、製品チームに目的地-フレキシビリティの概念を与えることで、製品の改善に役立てることができる。
Such models provide an interpretable signal, valid under all the contexts where the product teams would like to use them.
このようなモデルは、製品チームが使用したいすべてのコンテクストで有効な、解釈可能なシグナルを提供する。
...
具体的には、我々の分析では、各意味モデルは平均して、特化されたものよりも2倍のユースケースを生成することがわかった。

## Model Families モデルファミリー
...
私たちは、目的地、物件の価格、物件の場所、品質、日付、設備など、いくつかの旅行の側面を考慮し、組み合わせて、各側面に柔軟性レベルを割り当てるユーザーの嗜好プロファイルを構築するいくつかの機械学習モデルを構築します。
Models in this family work as a semantic layer.
このファミリーのモデルはセマンティック・レイヤーとして機能する。
As an example the Dates Flexibility model gives a measure of how flexible a user is about traveling dates.
例えば、「日付の柔軟性」モデルは、ユーザーが旅行の日付に対してどの程度柔軟であるかを示すものである。
...
ユーザーが柔軟であれば、日付の推奨は状況によっては適切かもしれないが、ユーザーが柔軟でない場合、日付の推奨は気が散って混乱を招く可能性があるため、表示されない。
...
また、融通の利かないユーザーに焦点を当て、価格動向や空席状況などの関連情報を使って、選択した日程を再強化することもできる。

### Traveller Context Models. トラベラーのコンテキストモデル。
...
これらはすべて、私たちが「トラベラー・コンテキスト」と呼ぶものの一例であり、制約や要件を定義する旅のテーマである。
...
これらのコンテキストのほとんどは、私たちのプラットフォームでは明示されておらず、指定できるものでも、ほとんどのユーザーは通常省略している。
...
したがって、ショッピング体験のできるだけ早い段階で、現在のユーザーのコンテクストを予測する、あるいは推測することは、非常に価値がある。
...
### Item Space Navigation Models. アイテムスペースナビゲーションモデル。
...
当社のインベントリーを閲覧するほとんどのユーザーは、日付、物件、ポリシー、ロケーションなど、いくつかの補足的、補完的なオプションや項目をナビゲートします。
...
スクロール、クリック、ソート、フィルタリングなどのさまざまなアクションを、ユーザーの好みに関する暗黙のフィードバックとして扱う。
...
これらのシグナルは、ユーザー履歴の中で最も関連性の高いアイテムへのアクセスを容易にしたり、インベントリの中で他の関連性の高いアイテムを表示したりするために使用される。

### User Interface Optimization Models. ユーザーインターフェースの最適化モデル。
...
フォントサイズ、リスト内の項目数、背景色や画像、ビジュアル要素の有無など、すべてがビジネス指標で測定されるユーザー行動に大きな影響を与えます。
...
そのため、私たちのモデルは、コンテキストとユーザー情報を考慮して、最適なユーザーインターフェースを決定します。

### Content Curation. コンテンツ・キュレーション。
...
特定の都市、日付、あるいは物件にゲストを誘致し、宣伝するために使用できるため、大きな可能性を秘めているが、非常に複雑で騒々しく、膨大であるため、ユーザーに消費されにくいという欠点もある。
Content Curation is the process of making content accessible to humans.
コンテンツ・キュレーションとは、コンテンツを人間がアクセスできるようにするプロセスである。
...
機械学習モデルがレビューを「キュレーション」し、宿泊施設の優れた点を簡潔かつ代表的に要約する（図1（b））。

### Content Augmentation. コンテンツの補強。
...
ユーザーが宿泊施設をブラウズし、選択し、予約し、レビューするプロセス全体は、特定の施設や目的地が提供できるサービスや品質をより深く理解することを可能にする暗黙のシグナルを私たちが自由に使えるようにする。
...
予約の時期、旅行日、目的地などによって、価格は異なる動きを見せる。 各都市の何千件もの予約に毎日アクセスしているため、指定された時間と旅行日程におけるその都市の価格動向の正確なモデルを構築することができます。 モデルが特定の傾向を見つけると、私たちはユーザーに情報を提供し、チャンスがありそうな目的地や日程を選ぶように促したり、特定の選択肢を控えて他の選択肢を優先させたりすることで、より良い決断ができるように支援する。 この場合、拡張されたアイテムは宿泊施設ではなく、目的地であることに注意（図1(c)参照）。

## All model families can provide value すべてのモデル・ファミリーが価値を提供できる
...
ほとんどの家庭の貢献度はベンチマークを上回っており、1家庭は下回っているが、どの家庭も大きな貢献をしており、総合的な効果は明らかにプラスである。
...
また、モデルが新製品の基礎となり、他の製品開発分野を通じて価値創造を可能にすることも観察されている。
...
一例として、図3は観光地推奨システムの開発における反復プロセスを示している。
...
ユーザーインターフェース、ターゲットオーディエンス、コピー（キャプション、説明文、メッセージなど）、またはアルゴリズムそのものです。
...
# Modeling: Offline Model Performance Is Just A Health Check モデリング 
オフラインでのモデルのパフォーマンスは健康診断に過ぎない
...
Booking.comでは、あるモデルがお客様と私たちのビジネスにもたらす価値を非常に重視しています。
...
このような価値は、ランダム化比較試験（RCT）や、コンバージョン、カスタマー・サービス・チケット、キャンセルなどの特定のビジネス指標を通じて推定される。
...
# Modeling:Before Solving A Problem, Design It モデリング：問題を解決する前に、それをデザインする
...
# Monitoring: Unsupervised Red Flags モニタリング 
監視されないレッドフラッグ
...
## Model-output dependent triggering モデル出力に依存するトリガー
...
## Controlling performance impact パフォーマンスへの影響をコントロールする
