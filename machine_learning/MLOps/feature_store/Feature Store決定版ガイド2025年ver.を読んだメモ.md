...
- Feature Storeの役割って、特に推薦タスクとかだと重要そうだなぁ...
  - 特に推薦などの意思決定最適化タスクでは、ユーザ行動を考慮した特徴量を使うことが多い。また、MLモデルの出力側(正解ラベルや報酬ログ)もタイムスタンプ付きのデータであることが多い。
  ...
  - それを踏まえると、推薦タスクなどではFeature Storeを導入するモチベーションは十分にあると思った。
    - (逆に言えば、特徴量がstaticなタスクだったり、学習時のデータリークが発生しづらいようなタスクだったら、Feature Storeを導入する必要はあんまりないのかも...?:thinking:)
- 
