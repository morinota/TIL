...
3. Call model.predict(..) with the transformed inference data. 
   1. 変換された推論データを使用してmodel.predict(..)を呼び出します。
...
また、**PySparkを使用してバッチ推論パイプラインをスケールアウトする方法**や、レイクハウステーブルに書き込む際のパフォーマンスを向上させるためにデータモデルをリファクタリングする方法についても見ていきます。

<!-- ここまで読んだ!    -->

### 1.1. Batch Predictions for a Time Range 時間範囲に対するバッチ予測
...
The start and end time parameters can be either a string or a datetime object.
...
When reading training data, the feature_view also creates a training_dataset_ver sion to store the query’s additional metadata. 
...
The training_dataset_version identifies the training data used to train a model. 
training_dataset_versionは、モデルのトレーニングに使用されたトレーニングデータを識別します。
...
### 1.2. Batch Predictions for Entities エンティティに対するバッチ予測

(基本的に推薦タスクの場合はこっちのケースが多そう...!:thinking:)
...
ただし、テーブルが時間でパーティション分けされていて、エンティティの最新の特徴データや特定の時間範囲にわたるエンティティの特徴データを取得したい場合、これはテーブル内のすべての行を処理します。
A full table scan is very expensive if the table contains a large number of rows. 
**テーブルに大量の行が含まれている場合、フルテーブルスキャンは非常に高価**です。(うんうん分かる...!:thinking:)
...
cc_trans_fgは**レイクハウステーブルであるため、カラムにZ-orderセカンダリインデックスを直接追加することができます（Apache HudiおよびDelta Lakeで）、パーティション内の行を順序付け**ます。
...
(まあバッチ推論をスケールさせるならマルチインスタンスなのかなぁ...:thinking:)

<!-- ここまで読んだ! -->

### 1.4. Data Modeling for Batch Inference バッチ推論のためのデータモデリング

Batch inference programs typically only process data from lakehouse tables. 
**バッチ推論プログラムは通常、レイクハウステーブルからのデータのみを処理します。** (うんうん。バッチシステムだからオンラインストアにアクセスする必要ないもんね...:thinking:)
It is impor‐ tant to understand certain properties of the open table formats (OTFs) to design more efficient data models. 
より効率的なデータモデルを設計するためには、**オープンテーブルフォーマット（OTFs）の特定の特性を理解することが重要**です。
...
Your cc_trans_fg table has many billions of rows, but your Spark job is only updating a few thousand rows. 
あなたのcc_trans_fgテーブルには数十億行がありますが、Sparkジョブは数千行しか更新していません。
...
これを克服するために、AvroファイルをParquetファイルにマージし、小さなParquetファイルをマージするために、バックグラウンド圧縮ジョブまたはテーブルサービスを（1時間/日/週に1回）実行するようにスケジュールできます。(S3 Tablesが自動でやってくれるやつね...!:thinking:)
...
しかし、**書き込み増幅を減らすもう一つの方法は、データモデルをリファクタリングして、更新を小さなテーブルに隔離すること**です。(更新頻度が異なる特徴量は、別のfeature groupに分けるのが良いって話かな...:thinking:)
...
### 1.5. Batch Inference for Neural Networks ニューラルネットワークのバッチ推論

Batch inference with deep learning models can benefit from GPU acceleration. 
**深層学習モデルを使用したバッチ推論は、GPUアクセラレーションの恩恵を受けることができます。** (これがNN系アプローチのメリットでもあるのか...!:thinking:)
...
**推論データのバッチサイズは、OOMエラーを避けるために利用可能なGPUメモリに合わせて調整する必要**があります。
...
### 1.6. Batch Inference for LLMs LLMのバッチ推論
...
**そのようなシンプルなプログラムは、バッチ推論データを読み込み、それをプロンプトテンプレートに適用し、バッチ推論リクエストをLLMに送信し、出力を推論ストアに保存**します。
The easiest way to get started is to use an LLM via an API. 
**始める最も簡単な方法は、APIを介してLLMを使用すること**です。
...
**LLMバッチ推論パイプラインを設計する際は、以下のステップを含める必要**があります：
...
2. For each row of batch inference data, use the prompt template to build a query that may contain one-shot or few-shot examples. 
   1. バッチ推論データの各行について、プロンプトテンプレートを使用して、ワンショットまたはフューショットの例を含む可能性のあるクエリを構築します。
...
3. Send your queries one at a time to the LLM API endpoint until all inference data has been processed (consider API rate limits, cost, and limits on the size of data the LLM will process for you per minute/hour). 
   1. **すべての推論データが処理されるまで、クエリを1つずつLLM APIエンドポイントに送信**します（APIのレート制限、コスト、およびLLMが1分/時間あたりに処理するデータのサイズの制限を考慮します）。
...
4. Save LLM responses to an inference store for analysis/processing or eagerly execute actions when a batch of responses is received. 
   1. LLMの応答を推論ストアに保存して分析/処理するか、応答のバッチを受信したときに即座にアクションを実行します。

Here is a more detailed code example that uses an OpenAI LLM to answer questions that arrived in the last 10 minutes. 
以下は、**過去10分間に到着した質問に答えるためにOpenAI LLMを使用**する詳細なコード例です。(まあ確かによくありそう...!!:thinking:)
...
これらの質問をLLMエンドポイントに送信し、レスポンスを**オフラインレスポンスフィーチャーグループに保存**して、後で使用できるようにします：
...
**私たちは、`questions`の回答列ではなく、別のフィーチャーグループに回答を書き込みます。これは、レイクハウステーブルに追加する方が、既存のレイクハウステーブルを更新するよりもはるかに効率的だから**です。
...
The `temperature` parameter controls the randomness of OpenAI’s model outputs. 
`temperature` パラメータは、OpenAIのモデル出力のランダム性を制御します。
...
**LLMを使用したバッチ推論のための新しいライブラリも登場**しており、例えば[fenic](https://oreil.ly/OG-oO)では、LLM推論がDataFrame上の列操作として行われます（map/classify/extract/semantic.join）。

<!-- ここまで読んだ! -->

## 2. Online Inference Pipelines オンライン推論パイプライン
...
###### 2.0.0.0.2. Ensure Offline-Online Consistency for Libraries
###### 2.0.0.0.3. ライブラリのオフライン-オンライン整合性を確保する
...
###### 2.0.0.0.4. Model Deployments with FastAPI
###### 2.0.0.0.5. FastAPIを使用したモデルのデプロイ
...
###### 2.0.0.0.8. Deployment API for Models and Feature Views
###### 2.0.0.0.9. モデルとフィーチャービューのためのデプロイメントAPI
...
###### 2.0.0.0.10. Model-Serving Frameworks with KServe
###### 2.0.0.0.11. KServeを使用したモデルサービングフレームワーク
...
###### 2.0.0.0.12. Performance and Failure Handling
###### 2.0.0.0.13. パフォーマンスと障害処理
...
###### 2.0.0.0.14. Mixed-Mode UDFs
###### 2.0.0.0.15. 混合モードUDFs
...
###### 2.0.0.0.16. Native UDFs and Log-and-Wait
###### 2.0.0.0.17. ネイティブUDFとログ・アンド・ウェイト
...
###### 2.0.0.0.18. Handling Failures in Online Inference Pipelines
###### 2.0.0.0.19. オンライン推論パイプラインにおける障害処理
...
###### 2.0.0.0.20. Model Deployment SLOs
###### 2.0.0.0.21. モデルデプロイメントのSLO
...
###### 2.0.0.0.22. Inference with Embedded Models
###### 2.0.0.0.23. 埋め込みモデルによる推論
...
###### 2.0.0.0.24. Embedded AI-Enabled Applications
###### 2.0.0.0.25. 埋め込みAI対応アプリケーション
...
-----
###### 2.0.0.0.26. Stream-Processing AI-Enabled Applications
###### 2.0.0.0.27. ストリーム処理AI対応アプリケーション
...
###### 2.0.0.0.28. UIs for AI-Enabled Applications in Python
###### 2.0.0.0.29. PythonにおけるAI対応アプリケーションのUI
...
###### 2.0.0.0.30. Summary and Exercises 要約と演習
