...
### Zhuoran Liu Leqi Zou Xuan Zou ##### Bytedance Inc. Bytedance Inc. Bytedance Inc.  
### Caihua Wang Biao Zhang Da Tang ##### Bytedance Inc. Bytedance Inc. Bytedance Inc.
### Bolin Zhu[∗] Yijie Zhu Peng Wu
##### Fudan University Bytedance Inc. Bytedance Inc.
### Ke Wang Youlong Cheng[†] ##### Bytedance Inc. Bytedance Inc.
...
∗Bytedance Inc.でのインターンシップ中に行った作業。
...
[2https://www.byteplus.com/en/product/recommend](https://www.byteplus.com/en/product/recommend)

Permission to make digital or hard copies of all or part of this work for personal or classroom use is granted without fee provided that copies are not made or distributed for profit or commercial advantage and that copies bear this notice and the full citation on the first page.  
この作業のすべてまたは一部のデジタルまたは印刷コピーを個人または教室で使用するために作成する許可は、利益または商業的利益のためにコピーが作成または配布されず、最初のページにこの通知と完全な引用が記載されている限り、無料で付与されます。

Copyrights for components of this work owned by others than ACM must be honored.  
ACM以外の他者が所有するこの作業のコンポーネントの著作権は尊重されなければなりません。

Abstracting with credit is permitted.  
クレジットを付けての要約は許可されています。

To copy otherwise, or republish, to post on servers or to redistribute to lists, requires prior specific permission and/or a fee.  
それ以外のコピー、再出版、サーバーへの投稿、リストへの再配布には、事前の特定の許可および/または料金が必要です。

Request permissions from permissions@acm.org.  
permissions@acm.orgに許可をリクエストしてください。

_ORSUM@ACM RecSys 2022, September 23rd, 2022, Seattle, WA, USA_  
_ORSUM@ACM RecSys 2022, 2022年9月23日, シアトル, WA, USA_
...
[5, 6, 10, 12, 20, 21] as the gigantic amount of user data is a natural fit for massively data-driven neural models.  
[5, 6, 10, 12, 20, 21] 巨大な量のユーザーデータは、大規模なデータ駆動型ニューラルモデルに自然に適合します。
...
#### 1.1 Sparsity and Dynamism スパース性と動的性  
-----
...
Embedding table Feature IDs lookup  
埋め込みテーブル 特徴IDのルックアップ
...
Feature IDs and gradients _interval_ Model  
特徴IDと勾配 _間隔_ モデル

forward  
フォワード

Apply pass Ranking Result gradient updates User Actions  
適用パス ランキング結果 勾配更新 ユーザーアクション
...
_Batch_ _Training_ _Stage_  
_バッチ_ _トレーニング_ _ステージ_

_Online_ _Training_ _Stage_  
_オンライン_ _トレーニング_ _ステージ_

|Col1|Feature IDs|Col3|Col4| 
|---|---|---|---| 
||Embedding table lookup Feature embeddings||| 
||Feature IDs and gradients||| 
||Apply gradient updates||| 
||Feature IDs||| 
|||Sync Embedding table lookup Sync interval|| 
|Model forward and backward pass Data of features and user reactions|||| 
||Embedding table lookup Feature embeddings||| 
||||Feature embeddings| 
||Feature IDs and gradients||| 
||||Model forward pass| 
...
高次元の埋め込み空間にマッピングする実践は、一連の問題を引き起こします。
...
実際には、多くのシステムがメモリフットプリントを削減し、IDの増加を許可する方法として低衝突ハッシング[3, 6]を採用しています。
...
したがって、生産規模の推薦システムには、そのパラメータにできるだけ多くの特徴をキャプチャする能力と、管理しようとするユーザーやアイテムの数を弾力的に調整する能力が求められます。

Training PS  
トレーニングPS

#### 1.2 Non-stationary Distribution 非定常分布  
...
その結果、ユーザーデータの基礎となる分布は非定常であり、一般的にコンセプトドリフト[8]と呼ばれる現象です。
...
これらのアーキテクチャの能力によって強化されたMonolithは、ほぼ同様のメモリ使用量で衝突を伴うハッシュトリックを採用したシステムを一貫して上回り、最先端の成果を達成します。

-----
Monolith: Collisionless Embedding Tableを用いたリアルタイム推薦システム ORSUM@ACM RecSys 2022, 2022年9月23日, シアトル, WA, USA  
online serving AUC without overly burdening our servers’ computation power.  
サーバーの計算能力に過度の負担をかけることなく、オンラインサービングAUCを実現します。
...
_T0_
_T1_
|Col1|Col2|Col3|h0(A)|Col5|Col6|Col7|Col8|Col9|Col10|Col11|Col12| |---|---|---|---|---|---|---|---|---|---|---|---| |||B A||||||D C|||G| |h1(D) h1(B) h0(C)|||||||||||| |D|||||E||F|||C B||  
-----
ORSUM@ACM RecSys 2022, September 23rd, 2022, Seattle, WA, USA Zhuoran, Leqi, Xuan, Caihua, Biao, Da, Bolin, Yijie, Peng, Ke, and Youlong  
Batch Training Data
HDFS  
