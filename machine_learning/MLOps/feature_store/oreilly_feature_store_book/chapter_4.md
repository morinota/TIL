...
つまり、**各行または特徴グループにTTLを指定でき、TTLが期限切れになると特徴データが削除**されます。

<!-- ここまで読んだ! -->

##### 1.5.3.1. Feature freshness 特徴の新鮮さ
...
**特徴グループ内の特徴データの新鮮さは、イベントが特徴パイプラインによって最初に読み取られてから、計算された特徴が推論パイプラインで使用可能になるまでの総時間**として定義されます（図4-7を参照）。
...
リアルタイムMLシステムの**新鮮な特徴は、通常、ストリームAPIを介して特徴ストアを更新するストリーミング特徴パイプラインを必要とします。** (妥協してマイクロバッチなのかな~:thinking:)
...
**もしそれが数秒ではなく数分かかった場合、TikTokのレコメンダーはリアルタイムであなたの意図を追跡しているようには感じられません。**AIはレコメンダーとして役に立つには遅すぎると感じられるでしょう。

<!-- ここまで読んだ! -->

##### 1.5.3.2. Data validation データ検証
...
### 1.6. Data Models for Feature Groups 特徴グループのデータモデル
...
**特徴ストアのデータモデリングは、以下を決定するプロセス**です：
...
**データモデルは、データベース理論からの用語**であり、データを異なる特徴グループ（テーブル）に分解する方法を指し、以下の目標を持っています：
...
Similarly, in feature stores, an entity maps to a feature group and relationships map to foreign keys in a feature group. 
...
**要件とデータソースからフィーチャーグループのデータモデル（エンティティ-リレーションシップ図など）に移行するプロセス**は何ですか？ 
...
- _Denormalization_ Improve query performance by increasing data redundancy and endangering data integrity. 
  - _非正規化_ **データの冗長性を増やし、データの整合性を危険にさらすことでクエリパフォーマンスを向上させます。** (これって、denormalizationっていうのか...!:thinking:)
...
これらの2つの技術は、**冗長（重複）データを含む非正規化データモデル**と**冗長データを排除する正規化データモデル**の2つのタイプに分類できるデータモデルを生成します。 
...
(へぇ〜DWHとかレイクハウスは、非正規化データモデルは結構許容する文化があるんだ...!!:thinking:)
...
エンティティ/フィーチャーのためのエンティティ、フィーチャー、およびフィーチャーグループを特定する前に、**フィーチャーデータを使用するAIシステムの種類を考慮**する必要があります:
...
**バッチとリアルタイムのクエリの両方に対して同様に機能する汎用データモデルを望む場合、次のセクションでスノーフレークスキーマ（正規化データモデル）がフィーチャーストアにおけるデータモデリングの好ましい方法論である**ことがわかります。(snowflake schema、よく知らないけど、SnowflakeはDWHだけど正規化データモデルの方法論なのかな...??:thinking:)
...
**スタースキーマとスノーフレークスキーマは、データをファクトテーブルに整理し、次元テーブルに接続するデータモデル**です。 
...
カラム型ストアで使用される他の人気のあるデータモデルには、_**データボールトモデル**（データの取り込みを効率的に処理するために使用され、データが遅れて到着したり、スキーマ変更が頻繁に発生する場合）_や、**1つの大きなテーブル（OBT）データモデル**（できるだけ多くのデータを単一の広いテーブルに保存することでデータモデリングを簡素化します）が含まれます。 
...
**OBTは、すべてのラベルとフィーチャーを単一の非正規化テーブルに保存するため、AIシステムには適しておらず、（行指向）オンラインストアでのストレージ要件が爆発的に増加し、時間とともに変化するフィーチャー値を保存するのにも適していません。** (OBTという方法論は、特徴量ストアには向かないのか...!:thinking:)
...
#### 1.6.1. Dimension Modeling with a Credit Card Data Mart  クレジットカードデータマートを用いた次元モデリング 
...
**データウェアハウジングで最も人気のあるデータモデリング技術は、次元モデル(dimension modeling)**です。これは、**データをファクトと次元に分類**します。
...
**次元はファクトの属性**です。一部の次元は時間とともに値が変化し、**SCD(slowly changing dimensions, 徐々に変化する次元)**と呼ばれます。
...
クレジットカード取引のデータマートにおけるファクトと次元の例を見てみましょう。**データマートは、特定のビジネスライン、チーム、または製品に焦点を当てたデータを含むデータウェアハウス（またはレイクハウス）のサブセット**です。 
...
私たちの例では、**クレジットカード取引がファクト**であり、次元はカード保有者、アカウントの詳細、銀行の詳細、商人の詳細など、クレジットカード取引に関するデータです。
(=イベントがfact、各イベントに紐づく属性情報がdimension、ってイメージでいいんだろうか...?? :thinking:)
...
_図4-8. クレジットカード取引のファクトと次元テーブル、スノーフレークスキーマデータモデルで整理されています。**テーブル間の線は、テーブルを相互にリンクする外部キー(foreign keys)**を表しています。たとえば、card_detailsには、カードを所有するアカウント（account_details）とカードを発行した銀行（bank_details）への参照が含まれています。_
...
_ファクトテーブルには、`credit_card_transactions、取引の一意のID`（t_id）、クレジットカード番号（cc_num）、取引のタイムスタンプ（ts）、支出額（amount）、商人のIPアドレス、および取引がオンラインか物理的かを示すコード（card_present）が保存されます。
...
- bank_details (銀行の詳細)
  - The bank’s credit_rating, its country, and the date when a row was last_modified 
  - 銀行の `信用格付け、国、` および行が最後に変更された日付。

- merchant_details (業者の詳細)
  ...
  - 前日に業者に対して発生したチャージバックの数（chargeback_prev_day）、業者のカテゴリコード（category）、その `国、` および行が最後に変更された日付。
...
クレジットカード取引テーブルは、**イベントソーシングパターン(event sourcing pattern)**を使用して構築されており、**1時間ごとにETL Sparkジョブが前の1時間にKafkaに到着したすべてのクレジットカード取引を読み取り、イベントを credit_card_transactions テーブルの行として永続化**します。(うんうん。ニュースアプリでいう記事閲読イベントとか...!:thinking:)
...
##### 1.6.1.1. Labels are facts, and features are dimensions ラベルは事実であり、特徴は次元です

(この理解しっくりきた!じゃあ意識せずにdimension modelingできてたみたい...!!:thinking:)
(ただ特徴量ストアと聞くと、特徴量=dimensionしか入れないイメージがあった。ラベル=factも入れるべきかな...??:thinking:)
...
フィーチャーストアでは、**factはモデルのラベル（またはターゲット/観測値）であり、特徴はラベルのdimensions**です。
Like facts, the labels are immutable events that often have a timestamp associated with them. 
**Factsと同様に、ラベルは不変のイベント**であり、しばしばそれに関連付けられたタイムスタンプがあります。(Factの定義 =　不変のイベント...?? まあ"事実"の意味的にも"不変"はしっくりくる:thinking:)
...
**これらの特徴はラベルのdimensionsであり、しばしば可変データ**です。
Sometimes they are SCDs, but in real-time ML systems, they might be fast-changing dimensions. 
**時にはそれらはSCD（ Slowly Changing Dimensions ）ですが、リアルタイムMLシステムでは、Fast-Changing Dimensions（急速に変化するDimensions）である可能性があります。** (SCDじゃなくてFCDである可能性もある...!:thinking:)
...
**特徴値が遅く変化するか早く変化するかにかかわらず、モデルのトレーニングデータとして特徴を使用したい場合は、すべての時点での特徴のすべての値を保存することが重要**です。
...
##### 1.6.1.2. Feature stores and SCD types　 フィーチャーストアとSCDタイプ
...
**SCD（SCDタイプ）を実装するための少なくとも5つのよく知られた方法**があり、それぞれがDimensionが変化するさまざまな方法に最適化されています。
...
つまり、フィーチャーストアは5つのSCDタイプすべてをサポートする必要はなく、**非常に特定のSCDタイプ（0、2、および4）**を必要とし、**フィーチャーグループ内でevent_time列を指定するだけで、それらのタイプをフィーチャーグループに目立たず追加できます。** (要するに、特徴量ストアの場合は通常のDWHとは異なり、even_timeカラムをつけるだけで十分にSCDのサポートができる、ってことか...!! 利用シナリオが限定されるから:thinking:)
...
表4-3は、フィーチャーストアがevent_timeを保存するフィーチャーグループ列を指定する比較的簡単なアプローチでSCDタイプ0、2、および4を実装する方法を示しています。

- メモ: テーブル4-3の内容
  - SCD type
    - Type 0: 
      - Usage: Immutable feature data (不変の特徴データ)
      - Description: No history is kept for feature data, so this type is suitable for features that are immutable. (特徴データの履歴は保持されないため、このタイプは不変の特徴に適しています。)
      - Feature store: Feature group, no event_time (**event_timeなしのフィーチャーグループ**)
    - Type 2:
      - Usage: Mutable feature data used by batch ML systems (バッチMLシステムで使用される可変特徴データ)
      - Description: When a feature value is updated for an entity ID, a new row is created with a new event_time (but the same entity ID). Each new row is a new version of the feature data. (エンティティIDの特徴値が更新されると、新しいevent_time（ただし同じエンティティID）で新しい行が作成されます。各新しい行は特徴データの新しいバージョンです。)
      - Feature store: Offline feature group with event_time (**event_timeを持つオフラインフィーチャーグループ**)
    - Type 4:
      - Usage: Online features for real-time ML systems; offline data for training (リアルタイムMLシステムのオンライン特徴; トレーニング用のオフラインデータ)
      - Description: Features are stored as records in two different tables—a table in the online store with the latest feature values and a table in the offline store with historical feature values. (特徴は2つの異なるテーブルにレコードとして保存されます—オンラインストアの最新の特徴値を持つテーブルと、オフラインストアの履歴的な特徴値を持つテーブルです。)
      - Feature store: Online/offline feature group with event_time (**event_timeを持つオンライン/オフラインフィーチャーグループ**)
