...
- **特徴量(Feature)はMLプロダクトの成否を左右する中心資産**だが、本番運用では管理・提供が一気に難しくなる。**特徴量ストア(Feature Store)**はその課題解消を担う重要コンポーネント
- SageMaker Feature Store を約4ヶ月試験運用した結果、NewsPicksの文脈では以下の4つの観点が特徴量ストアの重要観点だった:
...
NewsPicksではストレートニュース・オリジナル記事・動画コンテンツなど、多彩なコンテンツ種別が存在するため、ML機能もバッチ・リアルタイム共に増加傾向にあります。
またここ1年ほどで、事業部向かいのチームを含めてML活用の裾野が広がり、特徴量の共有・再利用・発見しやすさといった観点も無視できなくなってきました。

こうした背景から、私たちは特徴量ストアについて真剣に検討を始め、新しいML機能の開発に合わせてAWSのSageMaker Feature Storeを約4ヶ月間試験運用してみました。
...
本記事では、SageMaker Feature Store を約4ヶ月試験運用した経験をもとに、NewsPicksの文脈で特徴量ストアが真に価値を発揮するために重要だと感じた観点を整理しました。
