refs: https://medium.com/airbnb-engineering/t-leaf-taxonomy-learning-and-evaluation-framework-30ae19ce8c52
...
**タクソノミーは、情報を分類し整理するために使用される知識組織システム?(=knowledge organization system)**です。
Taxonomies use words to describe things — as opposed to numbers or symbols — and hierarchies to group things into categories. 
**タクソノミーは、物事を説明するために言葉を使用し（数字や記号ではなく）、階層を用いて物事をカテゴリにグループ化**します。(タグ付けとか巷で呼ばれてるやつね、"taxonomy"って表現するといいのか...!!:thinking:)
The structure of a taxonomy expresses how those things relate to each other. 
**タクソノミーの構造は、それらの物事がどのように相互に関連しているかを表現**します。(いいね...!:thinking:)
...
Airbnbは、**フロントエンド製品でタクソノミーを使用して、ゲストやホストが魅力的な宿泊や体験を発見し、インスピレーションを与えるコンテンツやカスタマーサポートの提供**を行います。(=これは目指したいね...!!:thinking:)
...
Airbnbは、**バックエンドツールでもタクソノミーを使用してデータを構造化し、内部情報を整理し、機械学習アプリケーションをサポート**します。(=うんうん、これも目指したいね...!!:thinking:)

<!-- ここまで読んだ! -->

Classifying the types of issues Airbnb community members face is vital for several reasons: 
Airbnbコミュニティメンバーが直面する問題の種類を分類することは、いくつかの理由から重要です。(これはAirbnbのコミュニティサポートの話...!:thinking:)
...
(↑の3つは、問い合わせに関するtaxonomyの重要なユースケースたち...!!:thinking:)

The Contact Reasons taxonomy is a new, consolidated issue taxonomy that supports all of these use cases. 
**Contact Reasonsタクソノミーは、これらすべてのユースケースをサポートする新しい統合されたissueタクソノミー**です。
...
(あ、以前はユースケース毎に同じような意味合いのtaxonomyが複数存在してたけど、それらをいい感じに統合したのがContact Reasonsってことね...!! めっちゃいいじゃん...!!:thinking:)

<!-- ここまで読んだ! -->

The consolidation of disjointed issue taxonomies into Contact Reasons was the first project of its kind at Airbnb. 
**分離されたissueタクソノミーをContact Reasonsに統合すること**は、Airbnbにおけるその種の最初のプロジェクトでした。
...
このような新しいタクソノミーの開発にはiterativeな学習が必要です：タクソノミストによってタクソノミーを作成/改訂すること; MLモデル、製品、サービスをトレーニングするために展開すること; タクソノミーの品質を評価し、改善のための領域を特定すること。
...
**タクソノミーの品質をより定量的かつ客観的に評価することで反復開発を加速するために、私たちはT-LEAF（Taxonomy Learning and Evaluation Framework）を作成し、カバレッジ、有用性、合意の3つの視点からタクソノミーを定量的に評価**します。
(あ、これが本ブログの主題なのか...!!:thinking:)
...
Airbnbコミュニティサポートの分野では、実世界のデータや明確な下流のワークフローアプリケーションがない状態で、**新しい分類法や分類ノード(=分類のユニークラベル的な意味...?:thinking:)を作成する必要があること**がよくあります。
...
**taxonomyは通常、質的中心のアプローチによって開発**されます¹。(うんうん、わかる...!!:thinking:)
...
新しいtaxonomyのプロトタイピングを始めたとき、既存のユーザからのフィードバックを評価し、洞察を得るためにゲストとホストを数回のユーザーリサーチに募集しました。
...
# Lengthy and iterative product cycle for taxonomy launches taxonomyの立ち上げのための長く反復的な製品サイクル
...
taxonomyの開発と立ち上げは、**実質的で信頼できる定量的フィードバックを得るために数四半期を要する長く反復的なプロセスになる可能性**があります。典型的なプロセスには以下が含まれます: (1 iterationが重く長くなりがちってことか...!:thinking:)
...
- Production changes to integrate backend environments and frontend surfaces, including necessary design and content updates
  - **必要なデザインとコンテンツの更新を含むバックエンド環境とフロントエンド表面**を統合するためのproductionの変更
...
T-LEAF以前は、**taxonomy開発プロセスは新しいtaxonomyの効果を測定するために出力メトリクスのみに依存**していました。これは、**1）大きな変更には実験とテストに長い時間がかかること、2）新しいノードの追加や更新などの小さな変更はテストされないこと**を意味します。これらの2つの痛点は、一貫した定期的なスコアリングによってT-LEAFフレームワークで対処できます。
...
T-LEAFフレームワークは、**タクソノミーの質を3つの側面で測定**します：1) カバレッジ、2) 有用性、3) 一致。
...
**カバレッジは、taxonomyが実世界のオブジェクトの範囲をどれだけうまく分類できるかを示します。**
...
「Contact Reasons」では、カバレッジスコアが、ゲストとホストがAirbnbのコミュニティサポートチームに連絡する理由をtaxonomyがどれだけうまく捉えているかを評価します。
When ‘coverage’ is low, a lot of user issues (data objects) will not be covered by the taxonomy and become ‘Other’ or ‘Unknown’. 
**「カバレッジ」が低いと、多くのユーザーの問題（データオブジェクト）がtaxonomyにカバーされず、「その他」または「不明」**となります。
...
**有用性は、オブジェクトが意味のあるカテゴリに分類されるタクソノミーの構造全体にどれだけ均等に分布しているか**を示します。
...
**一方、タクソノミーがあまりにも細かすぎると、オブジェクト間の類似点を説明できなくなる可能性**があります。
...
**T-LEAFでは、n個の例（例えば、異なるユーザの問題）を持つベンチマークデータセットに対して、ノード数が$\sqrt{n}$のタクソノミーが「粗すぎる」と「細かすぎる」の間で良いバランスを提供すると仮定**します。(これくらいのノード数があると、ユーザの問題を適切に区別できるくらいの粒度になるのでは、という仮説に基づいてるのね...!!:thinking:)
...
- x: number of proposed taxomony nodes (提案されたタクソノミーノードの数)
- $\mu$: controls expected barance, default as $\sqrt{n}$ (期待されるバランスを制御するパラメータ、デフォルトは$\sqrt{n}$)
- n: number of examples in the benchmark dataset (ベンチマークデータセットの例の数)
- $\sigma$: controls the degree of usefullness (有用性の程度を制御するパラメータ)
...
例えば、100の異なるユーザの問題がある場合、1（‘粗すぎる’）または100カテゴリ（‘細かすぎる’）に分割すると、有用性スコアは0に近くなります。
...
- Agreementに関する個人メモ:
  - 「**このtaxonomy(=分類ルール)、誰がやっても同じになる??**」っていうのを定量的に評価するための指標。基本的にはAgreementが高いほど、taxonomyの定義が明確で良いと判断できる...!! :thinking:
  - 2種のAgreement metricsを提案してる。
    - 1. human-human agreement: 
      - 同じデータセットに複数の人間のアノテーターがアノテーションを付ける。どれくらい一致するか。
      - (ブログ内では高価、と言われているが、現代であればLLMを駆使して、LLM-LLM agreement的な活用ができそう...!!:thinking:)
    - 2. human-model agreement:
      - 単一の人間のアノテーターがアノテーションを付けたデータセットでMLモデルをトレーニングする。モデルのトレーニング精度を測定する。taxonomyのノードが人間にとって十分に明確であれば、モデルは人間の意図をよりよく捉えることができるはず。
  - Agreementの意味合い:
    - Agreementが低い = taxonomyが壊れている。
    - 理由:
      - 1. 定義が曖昧。
        - ノード間の境界が不明確。
      - 2. 粒度が変。
        - 粗過ぎor細か過ぎ。
  - ニュース系の例:
    - 記事「日銀が金利据え置き」に関するtaxonomy。
      - 人A「金融政策!」
      - 人B「日銀!」
      - モデル「金融!」
      - → この場合、Agreementの値は低くなる...!
    - これは何が悪い??
      - taxonomyの定義が曖昧。
      - 粒度がバラバラ
  - **Agreementはtaxonomyの設計ミスを暴く指標**、と言える...!!
  - 実務でのagreementの計算方法 = cohen's kappa:
    - ざっくり「偶然の一致率を除いた、一致率」を図るための指標。
    - 値域は[-1, 1]で、1に近いほど一致度が高く、0に近いほど偶然の一致と同程度、-1に近いほど逆にズレてる。
    - $kappa = \frac{p_o - p_e}{1 - p_e}$
    - ここで
      - $p_o$ = 実際に観測された一致率
      - $p_e$ = 偶然の一致率
      - 一致率 = 2人の評価者が同じラベルを付けた割合。
...
**複数の人間のアノテーターが、分類法の定義に従って同じデータにアノテーションを付け**、私たちはCohen Kappaを使用して評価者間の信頼性を[-1, 1]の範囲で計算します。

$$
keppa = \frac{(total_accuracy - random_accuracy)}{(1 - random_accuracy)}
$$

### ML Model Training Accuracy MLモデルのトレーニング精度

Having multiple human raters annotate one data set can be expensive. 
**複数の人間の評価者が1つのデータセットにアノテーションを付けることは高価になる可能性**があります。(うんうん、手間だよね...!! でも今はLLMがアノテーたーとして振る舞ってくれそうだから実現可能性も高いか...!!:thinking:)
In reality, most data is annotated by just one human. 
実際には、ほとんどのデータは1人の人間によってアノテーションが付けられています。 
...
**タクソノミーが明確に定義されている場合（すなわち、高い「合意」を持つ場合）、類似の問題（データポイント）は、これらのラベルが異なるエージェントから来ているにもかかわらず、類似のラベルを持つべき**です。 
...
マルチレーターデータセットを作成するのに、1人のアナリストと9人のアノテーション者が約1ヶ月かかりました。 
...
**表1に示されているように、MLトレーニング精度はタクソノミーの「合意」品質の類似した評価を提供**します。

![]()

<!-- ここまで読んだ! -->

# Developing the Contact Reason Taxonomy using T-LEAF T-LEAFを使用してcontact reason taxonomyを開発する

**連絡理由の分類法は、レベル1（L1）の広いカテゴリからレベル2（L2）の狭いカテゴリ、そしてレベル3（L3）の特定の問題に至るまで、約200のノードで構成**されています。例えば：
...
古い分類法は、セクションによって予測不可能な粒度のレベルを持っていましたが、contact reasonは、継続的な評価フレームワークをより適切にサポートする**一貫した3レベルの構造**を持っています。
...
私たちは、**古い分類法から新しい分類法（連絡理由）への移行にT-LEAFを利用し、より迅速なフィードバックループを可能にし、新しい分類法を本番環境に投入する前に定量的な品質管理を提供**しました（図3）。
...
**その入力を使用して、Core Machine Learning（CoreML）エンジニアリングチームと分類法チームは、本番環境での実験を行う前にT-LEAFスコアを大幅に改善するために協力**しました。
...
これらの本番環境の1つでContact Reasonsタクソノミーを評価するために、Airbnb bot³でのパフォーマンスを確認しました。(ここからオンライン評価ね...!!:thinking:)
...
私たちは、T-LEAFのカバレッジ、有用性、合意の指標で測定された連絡理由の分類法の改善が、問題のカバレッジ、自己解決の効果、および問題予測の精度の実際の改善にもつながることを発見しました。(=抽象化すると、**オフライン評価指標の改善が、オンライン評価指標の改善につながることを確認できた**、ということね...!!:thinking:)

![Table 2. T-LEAF scores between old and new taxonomies]()

<!-- ここまで読んだ! -->

## A higher T-LEAF coverage score leads to greater issue coverage in production T-LEAFカバレッジスコアが高いほど、製品における問題のカバレッジが向上する
...
## A higher usefulness score leads to more issues being resolved through self-service より高い有用性スコアは、自己サービスを通じてより多くの問題が解決されることにつながります。
...
## A higher T-LEAF agreement score results in more accurate issue prediction T-LEAF合意スコアが高いほど、問題予測がより正確になります。
...
**taxonomyを評価するための定量的なフレームワークは、より迅速な反復をサポートし、主要な分類法の変革を開始するリスクを減らし、すべてのオーディエンス（ゲスト、ホスト、サポートアンバサダー、およびAirbnbビジネス）にプラスの影響**を与えます。
...
カバレッジ、有用性、合意の観点から分類法の質をスコアリングするT-LEAFフレームワークは、現在、コミュニティサポートのproduction taxonomyに適用されており、この定量的なtaxonomy評価の方法論を使用することで、より良いモデルパフォーマンスとより大きな問題のカバレッジにつながることが結果に示されています。
...
**T-LEAFをtaxonomyの進化のための継続的な改善フレームワークの一部として開発、試行、および確立することは、チーム全体の協力努力**でした。
...
contact reason taxonomyでこの新しい方法を試行したことで、**今後のtaxonomyイニシアチブにT-LEAFの方法論を適用し続けることで、より多くのプラスの結果が得られる**と確信しています。
