...
Recommending YouTube videos is extremely challenging from three major perspectives: • Scale: Many existing recommendation algorithms proven to work well on small problems fail to operate on our scale.
...
行列分解法の膨大な研究 [19]とは対照的に、推薦システムにディープニューラルネットワークを使用した研究は比較的少ない。
...
協調フィルタリングは、[22]ではディープニューラルネットワークとして、[18]ではオートエンコーダとして定式化されている。
...
セクション3では、候補生成モデルがどのように学習され、どのようにレコメンデーションを提供するために使用されるかを含め、より詳細に説明する。
...
セクション4は、どのように古典的なロジスティック回帰が（クリック確率ではなく）期待されるウォッチタイムを予測するモデルを訓練するために修正されるかを含む、ランキングモデルの詳細である。
...
このシステムは2つのニューラルネットワークで構成されている： 
1つは候補生成用、もう1つはランキング用である。
...
リストの中で少数の「ベスト」な推奨を提示するには、高い想起を持つ候補の中で相対的な重要性を区別するための細かいレベルの表現が必要である。
...
ランキング・ネットワークは、ビデオとユーザーを説明する豊富な特徴セットを使用して、望ましい目的関数に従って各ビデオにスコアを割り当てることによって、このタスクを達成する。
...
レコメンデーションの2段階アプローチにより、非常に大規模なコーパス（数百万本）からレコメンデーションを行うことができ、同時に、デバイスに表示される少数のビデオが、ユーザーにとってパーソナライズされた魅力的なものであることを確信することができる。
...
さらに、この設計は、以前の研究[3]で説明されたような、他のソースから生成された候補をブレンドすることを可能にする。
...
開発中、私たちはオフライン・メトリクス（精度、リコール、ランキング・ロスなど）を広範囲に活用し、システムの反復的な改善を導いている。
...
しかし、アルゴリズムやモデルの有効性を最終的に判断するためには、ライブ実験によるA/Bテストに頼ることになる。
...
ライブのA/B結果は、オフラインの実験と必ずしも相関関係がないため、これは重要である。

# Candidate Generation 候補者の生成
...
私たちのニューラルネットワークモデルの初期の反復は、ユーザーの過去の時計を埋め込むだけの浅いネットワークで、この因数分解の動作を模倣した。
From this perspective, our approach can be viewed as a nonlinear generalization of factorization techniques.
この観点から、我々のアプローチは因数分解技術の非線形一般化とみなすことができる。
...
ここでは、ユーザUとコンテキストCに基づいて、コーパスVから数百万のビデオi（クラス）の中から、時刻tに特定のビデオウォッチwtを正確に分類することが予測問題となる、
...
ここで、u∈R Nはユーザーとコンテキストのペアの高次元の「埋め込み」を表し、vj∈R Nは各候補ビデオの埋め込みを表す。
...
この設定では、埋め込みとは、単に疎なエンティティ（個々のビデオ、ユーザーなど）をR Nの密なベクトルにマッピングすることである。
...
ディープ・ニューラル・ネットワークのタスクは、ソフトマックス分類器で動画を識別するのに有用な、ユーザーの履歴とコンテキストの関数としてのユーザー埋め込みuを学習することである。
...
YouTubeには、明示的なフィードバック・メカニズム（サムズアップ／ダウン、商品内アンケートなど）が存在するが、私たちはモデルをトレーニングするために、ユーザーがビデオを完走したことを肯定的な例とする、時計の暗黙的フィードバック[16]を使用する。
...
この選択は、利用可能な暗黙のユーザー履歴が桁違いに多いことに基づいており、明示的なフィードバックが極端に少ない尻尾の奥深くまでレコメンデーションを作成することを可能にしている。

### Efficient Extreme Multiclass 効率的なエクストリーム・マルチクラス
...
何百万ものクラスでこのようなモデルを効率的に訓練するために、我々は背景分布から負のクラスをサンプリングし（「候補サンプリング」）、重要度重み付けによってこのサンプリングを補正する技術に頼る[10]。
...
サービング時に、ユーザーに提示するトップNを選択するために、最も可能性の高いN個のクラス（ビデオ）を計算する必要がある。
...
数十ミリ秒の厳しい待ち時間の下で数百万のアイテムをスコアリングするには、クラス数に比例しない近似的なスコアリング方式が必要である。
...
ソフトマックス出力層からの較正された尤度は、サービング時には必要ないので、スコアリング問題は、汎用のライブラリを使用することができるドット積空間の最近傍探索に軽減される[12]。
...
連続的なBag of Word言語モデル[14]にヒントを得て、各動画の高次元の埋め込みを固定語彙で学習し、これらの埋め込みをフィードフォワード・ニューラル・ネットワークに入力する。
...
ユーザーの視聴履歴は、スパースなビデオIDの可変長シーケンスで表現され、エンベッディングを介して密なベクトル表現にマッピングされる。
...
このネットワークは、固定サイズの密な入力を必要とし、いくつかの戦略（合計、成分ごとの最大値など）の中で、単純に埋め込みを平均化することが最も良い結果を出した。重要なことは、埋め込みは、通常の勾配降下バックプロパゲーション更新を通じて、他のすべてのモデルパラメータと共同で学習されることである。
...
特徴は広い第1層に連結され、その後に完全連結された整流線形ユニット（ReLU）[6]の数層が続く。
...
ディープニューラルネットワークを行列分解法の一般化として使用する主な利点は、任意の連続的特徴やカテゴリー的特徴を簡単にモデルに追加できることである。
...
ユーザーの性別、ログイン状態、年齢のような単純なバイナリおよび連続的特徴は、[0, 1]に正規化された実数値としてネットワークに直接入力される。

### “Example Age” Feature 年齢例」機能
...
最近アップロードされた（「新鮮な」）コンテンツを推薦することは、商品としてのYouTubeにとって非常に重要である。
We consistently observe that users prefer fresh content, though not at the expense of relevance.
私たちは一貫して、ユーザーが新鮮なコンテンツを好むことを観察しているが、関連性を犠牲にしているわけではない。
...
ユーザーが見たい新しいビデオを単純に推薦するという一次的な効果に加え、バイラルコンテンツをブートストラップして伝播させるという重要な二次的現象がある[11]。
...
ビデオの人気度分布は非定常性が高いが、我々のレコメンダーが生成するコーパス上の多項分布は、数週間のトレーニングウィンドウにおける平均視聴可能性を反映する。
To correct for this, we feed the age of the training example as a feature during training.
これを補正するために、訓練中に訓練例の年齢を特徴として与える。
...
サービング時には、モデルがトレーニング・ウィンドウの最後の方で予測を行っていることを反映するため、この特徴はゼロ（またはわずかにマイナス）に設定される。
...
## Label and Context Selection ラベルとコンテキストの選択
...
我々は、この代理学習問題の選択が、A/Bテストのパフォーマンスにおいて非常に重要であるが、オフライン実験では測定が非常に困難であることを発見した。
...
トレーニングの例は、私たちが作成したレコメンデーションの視聴だけでなく、すべてのYouTubeの視聴（他のサイトに埋め込まれているものも含む）から生成されます。
...
そうでなければ、新しいコンテンツが浮上するのは非常に難しく、レコメンダーは過度に搾取に偏ることになる。
...
もしユーザーが私たちの推薦以外の手段でビデオを発見した場合、協調フィルタリングによってこの発見を素早く他のユーザーに伝えられるようにしたい。
...
ライブメトリクスを改善するもう一つの重要な洞察は、ユーザーごとに固定数のトレーニング例を生成し、損失関数でユーザーを均等に重み付けすることでした。
This prevented a small cohort of highly active users from dominating the loss.
これにより、少数のアクティブなユーザーが損失を独占することを防いだ。
...
やや直感に反するが、モデルがサイトの構造を利用し、サロゲート問題をオーバーフィットするのを防ぐために、分類器から情報を隠すように細心の注意を払わなければならない。
...
我々の問題は、次に視聴される動画を予測することであるため、この情報を与えられた分類器は、視聴される可能性が最も高い動画は、「taylor swift」の対応する検索結果ページに表示される動画であると予測する。
Unsurpisingly, reproducing the user’s last search page as homepage recommendations performs very poorly.
驚くことに、ユーザーが最後に検索したページをホームページのレコメンデーションとして再現するのは、非常にパフォーマンスが悪い。
...
シーケンス情報を破棄し、検索クエリを順序付けされていないトークンの袋で表現することで、分類器はラベルの由来を直接意識することがなくなる。
Natural consumption patterns of videos typically lead to very asymmetric co-watch probabilities.
ビデオの自然な消費パターンは通常、非常に非対称な共同視聴確率をもたらす。
...
エピソードシリーズは通常、順次視聴され、ユーザーは多くの場合、あるジャンルのアーティストを最も広く人気のあるものから発見し、その後、より小さなニッチに焦点を当てる。
...
そのため、ランダムに保留された時計を予測するよりも、ユーザーの次の時計を予測する方がはるかに優れたパフォーマンスを示すことがわかった（図5）。
...
対照的に、ランダムな時計を選択し、保留されたラベルの時計の前にユーザーが取った行動のみを入力することで、ユーザーの履歴を「ロールバック」する（5b）。

## Experiments with Features and Depth 特徴と深さの実験

Adding features and depth significantly improves precision on holdout data as shown in Figure 6.
図6に示すように、特徴量と深さを追加することで、ホールドアウトデータの精度が大幅に向上する。
...
これらの実験では、1Mのビデオと1Mの検索トークンの語彙を、それぞれ256個のフロートで、最近のウォッチ50個と最近の検索50個の最大バッグサイズに埋め込んだ。
...
ソフトマックス層は、256次元の同じ1Mビデオクラス（これは別の出力ビデオ埋め込みと考えることができる）上の多項式分布を出力する。
...
ネットワーク構造は一般的な "タワー "パターンに従っており、ネットワークの下部が最も広く、各隠れ層はユニット数が半分になる（図3と同様）。
...
デプスゼロ・ネットワークは、事実上、線形因数分解スキームであり、前身のシステムと非常によく似た性能を発揮した。
Width and depth were added until the incremental benefit diminished and convergence became difficult: • Depth 0: A linear layer simply transforms the concatenation layer to match the softmax dimension of 256 • Depth 1: 256 ReLU • Depth 2: 512 ReLU → 256 ReLU • Depth 3: 1024 ReLU → 512 ReLU → 256 ReLU • Depth 4: 2048 ReLU → 1024 ReLU → 512 ReLU → 256 ReLU
幅と深さは、増分的な利益が減少し、収束が困難になるまで追加された： 
- 深さ0： 
深さ0：線形レイヤーは、単純に256のソフトマックス次元に合わせて連結レイヤーを変換する： 
256 ReLU - 深さ2： 
512 ReLU → 256 ReLU - 深度3： 
1024 ReLU → 512 ReLU → 256 ReLU - 深度4： 
...
### Embedding Categorical Features カテゴリー特徴の埋め込み
...
### Normalizing Continuous Features 連続特徴の正規化
...
## Modeling Expected Watch Time 時計の予想時間をモデル化する
...
## Experiments with Hidden Layers 隠しレイヤーの実験
