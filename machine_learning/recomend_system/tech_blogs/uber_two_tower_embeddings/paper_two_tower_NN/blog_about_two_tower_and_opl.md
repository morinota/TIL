...
- 推薦システムの実務で広く使われるTwo-Towerモデルを、書籍「反実仮想機械学習」を参考にIPS推定量やDR推定量を使った勾配ベースのオフ方策学習で最適化させてみた。
- PytorchでTwo-Towerモデルを実装し、open bandit pipelineパッケージの合成データを使用して実験を行った。
