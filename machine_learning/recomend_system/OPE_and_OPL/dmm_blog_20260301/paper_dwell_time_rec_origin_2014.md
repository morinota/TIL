...
**ユーザーの行動** **クライアントサイドのイベント**

A user opens a news article page. 
ユーザーがニュース記事のページを開きます。

_{DOM-ready, t1}_ 
_{DOM準備完了, t1}_

He reads the article for several _{Focus, t2}_ seconds. 
彼は数秒間記事を読みます _{Focus, t2}_。

He switches to another browser _{Blur, t3}_ tab or a window to read other articles. 
彼は他の記事を読むために別のブラウザのタブ _{Blur, t3}_ またはウィンドウに切り替えます。

He goes back to the article page _{Focus, t4}_ and comments on it. 
彼は記事ページに戻り _{Focus, t4}_、コメントをします。
...
## 4. MEASURING ITEM DWELL TIME アイテム滞在時間の測定
...
ウェブスケールのユーザーブラウジング活動データからアイテムレベルの滞在時間を正確に計算することは、難しい問題です。 
...
ターゲットページがユーザーの注意を引く時間を計算することができます。 
...
## 6. 3.2 Dwell Time Analysis 滞在時間分析
...
その性質を理解するために、私たちはYahooからの大規模な実世界データコレクションから、アイテムごとのユーザごとの滞在時間を分析します。
...
デスクトップ、タブレット、モバイルデバイスなど、異なるデバイスにおける記事の長さに対する滞在時間の散布図を示します。
...
図から、記事の長さがデバイス全体での平均滞在時間と良好な線形相関を持つことが非常に明らかです。
...
言い換えれば、コンテンツの長さだけではユーザごとのアイテムごとの滞在時間をほとんど説明できず、長いコンテンツストーリーに対する滞在時間に基づくユーザエンゲージメント測定のバイアスに注意する必要があります。
...
スライドショーやビデオなどの他のコンテンツタイプについては、コンテンツの長さはスライドショーのスライド数やビデオクリップの生の長さである可能性があります。
...
**LogDwellTime**
**Figure 5: The (un)normalized distribution of log of dwell time** 
**for slideshows across different devices. The X-axis is the log of** 
**dwell time and the Y-axis is the counts (removed for proprietary** 
**reasons).**

**LogDwellTime**
**図5: スライドショーの滞在時間の対数の（非）正規化分布** 
**異なるデバイス間の。X軸は滞在時間の対数で、Y軸はカウントです（** 
**商業上の理由で削除されています）。**
...
**LogDwellTime**
**図6: 異なるデバイスにおける動画の滞在時間の対数の（非）正規化分布。X軸は滞在時間の対数、Y軸はカウントです。**  
...
スライドショーに関しては、写真やスライドの数が多いほど、すべてのユーザーに対する平均滞在時間が長くなるという自然な仮定があります。 
...
例えば、過去のデータでは、ユーザーはデスクトップよりもモバイルやタブレットデバイスでの記事あたりの滞在時間が平均して短いことがわかりました。 
...
比較可能なユーザーエンゲージメント信号を抽出するために、異なるデバイスでのユーザーの異なるコンテンツ消費行動を扱うために正規化ユーザー滞在時間を導入します。 
...
ここで議論されている技術は、複数のコンテンツソース（例：スライドショーや記事）を統一されたストリームに統合するためにも使用できます。 
...
各コンテンツタイプに対するユーザーのアイテムごとの滞在時間の分布（すべてのユーザーから）は異なりますが、各コンテンツタイプの分布は長期間にわたって類似していることがわかりました。 
...
2. 新しいコンテンツアイテムiの文脈Ciにおける時間tIを考慮し、対数空間でz値を計算します：$z_i = \log(t_{i}^{\sigma}C_i) - \mu_{C_i}$。 

3. Compute the normalized dwell time of item i in the article space: ti,article = exp(µarticle + σarticle × zi). 
3. アイテムiの正規化滞在時間を記事空間で計算します：$t_{i,\text{article}} = \exp(\mu_{\text{article}} + \sigma_{\text{article}} \times z_i)$。 
...
コンテンツアイテムの平均滞在時間は、そのアイテムの固有の特性の一つと見なすことができ、ユーザーがそのアイテムにどれだけの時間を費やすかに関する重要な平均ユーザーエンゲージメント情報を提供します。
...
したがって、予測された平均滞在時間を活用することで、「カバレッジ」を大幅に改善できる（または欠損データの問題を軽減できる）可能性があります。
...
この小節では、シンプルな特徴を使用して記事のストーリーの滞在時間を予測するための機械学習手法を紹介します。
...
これらの特徴の重みを、各特徴が記事の平均滞在時間予測にどれだけ寄与するかとして大まかに解釈できます。
...
特徴の重みは、平均的なユーザーの記事読書行動に対する現在の期待と一致しています：長い記事は、より高い予測平均滞在時間につながる可能性があります。

people spend a longer time reading articles on desktop devices than mobile devices; 
人々は、モバイルデバイスよりもデスクトップデバイスで記事を読むのに長い時間を費やします。

more serious topics can lead users to dwell longer. 
より深刻なトピックは、ユーザーがより長く滞在することにつながる可能性があります。
...
## 11. USE CASE I: LEARNING TO RANK
## 12. 使用例 I: ランキング学習
...
安価で簡単なアプローチの1つは、ユーザーのクリックデータを関連性判断として使用することです。 
...
**MLRのための滞在時間: 滞在時間をMLRフレームワークに組み込むための2つの直感的な方法があります。**
rate dwell time into MLR frameworks. 
滞在時間をMLRフレームワークに組み込むための2つの直感的な方法があります。 
...
2) 記事ごとの滞在時間をサンプルインスタンスの重みとして使用し、$w_i = h(\gamma_d)$と扱います。ここで、関数$h$は滞在時間の変換です。 
...
すべての実験で、$h = \log(x)$が最良のパフォーマンスを発揮することがわかりました。
...
最初の観察は、滞在時間を学習ターゲットまたはインスタンス重みとして使用するいずれの方法も、3つの主要なランキング指標を改善できるということです。 
The second observation is that, dwell time as an instance weight leads to the best performance. 
2つ目の観察は、インスタンス重みとしての滞在時間が最良のパフォーマンスをもたらすということです。 
...
2013年6月から2013年8月の3か月間のバケット指標を報告します。 
...
最初は、3つのバケットが同じ線形モデルを実行しており、最初の3つのデータポイント（3日間のデータ）から、CTRとユーザーエンゲージメント指標の両方が類似していることがわかります。 
...
2) B: GBDTモデルがクリックを最適化します。 
3) C: a GBDT model optimizes dwell time $h(\gamma_d)$. 
3) C: GBDTモデルが滞在時間$h(\gamma_d)$を最適化します。 
...
一つの考えられる原因は、高いCTRではなく滞在時間に基づくエンゲージメント信号を最適化する際に、ユーザーが推薦されたコンテンツをより好み、サイトに戻ってきてさらにクリックする可能性があることです。 
...
## 13. USE CASE II: COLLABORATIVE FILTERING
...
滞在時間とクリックのトレーニングプロセスの違いは、負の例のブートストラッププロセスです。 
...
すべての評価方法とすべてのメトリクスにおいて、滞在時間を最適化するモデルはクリックを最適化するモデルと同等のパフォーマンスを持つことが観察できます。 
...
全体的な改善が小さい一因として、コンテンツの特徴やユーザー側の情報が滞在時間に基づく評価をより良く予測するために必要かもしれません。 
...
## 14. DISCUSSION AND CONCLUSIONS 議論と結論
...
[1] D. Agarwal, B.-C. Chen, P. Elango, and R. Ramakrishnan.  
Content recommendation on web portals. Communications _of the ACM, 56(6):92–101, June 2013._  
ウェブポータルにおけるコンテンツ推薦。ACMの通信、56(6):92–101、2013年6月。

[2] J. Bian, A. Dong, X. He, S. Reddy, and Y. Chang.  
User action interpretation for online content optimization. IEEE _TKDE, 25(9):2161–2174, Sept 2013._  
オンラインコンテンツ最適化のためのユーザーアクション解釈。IEEE TKDE、25(9):2161–2174、2013年9月。

[3] C. Burges, T. Shaked, E. Renshaw, A. Lazier, M. Deeds, N. Hamilton, and G. Hullender.  
Learning to rank using gradient descent. In Proceedings of ICML, pages 89–96, 2005.  
勾配降下法を用いたランキング学習。ICMLの論文集、ページ89–96、2005年。

[4] Z. Cao, T. Qin, T.-Y. Liu, M.-F. Tsai, and H. Li.  
Learning to rank: From pairwise approach to listwise approach. In _Proceedings of ICML, pages 129–136, 2007._  
ランキング学習：ペアワイズアプローチからリストワイズアプローチへ。ICMLの論文集、ページ129–136、2007年。

[5] Y. Freund, R. Iyer, R. E. Schapire, and Y. Singer.  
An efficient boosting algorithm for combining preferences. The Journal _of Machine Learning Research, 4:933–969, Dec. 2003._  
好みを組み合わせるための効率的なブースティングアルゴリズム。機械学習研究のジャーナル、4:933–969、2003年12月。

[6] J. H. Friedman.  
Stochastic gradient boosting. Computational _Statistics & Data Analysis - Nonlinear methods and data_ _mining, 38(4):367–378, Feb. 2002._  
確率的勾配ブースティング。計算統計とデータ分析 - 非線形手法とデータマイニング、38(4):367–378、2002年2月。

[7] Y. Hu, Y. Koren, and C. Volinsky.  
Collaborative filtering for implicit feedback datasets. In Proceedings of ICDM, pages 263–272, 2008.  
暗黙のフィードバックデータセットのための協調フィルタリング。ICDMの論文集、ページ263–272、2008年。

[8] Y. Kim, A. Hassan, R. W. White, and I. Zitouni.  
Modeling dwell time to predict click-level satisfaction. In Proceedings _of WSDM, pages 193–202, 2014._  
滞在時間をモデル化してクリックレベルの満足度を予測する。WSDMの論文集、ページ193–202、2014年。

[9] Y. Koren, R. Bell, and C. Volinsky.  
Matrix factorization techniques for recommender systems. Computer, 42(8):30–37, Aug. 2009.  
レコメンダーシステムのための行列分解技術。コンピュータ、42(8):30–37、2009年8月。

[10] H. Li.  
Learning to rank for information retrieval and natural language processing. Synthesis Lectures on Human _Language Technologies, 4(1):1–113, 2011._  
情報検索と自然言語処理のためのランキング学習。人間言語技術に関する合成講義、4(1):1–113、2011年。

[11] C. Liu, J. Liu, N. Belkin, M. Cole, and J. Gwizdka.  
Using dwell time as an implicit measure of usefulness in different task types. Proceedings of the American Society for _Information Science and Technology, 48(1):1–4, 2011._  
異なるタスクタイプにおける有用性の暗黙的指標としての滞在時間の使用。アメリカ情報科学技術協会の論文集、48(1):1–4、2011年。

[12] T.-Y. Liu.  
Learning to rank for information retrieval.  
_Foundations and Trends in Information Retrieval,_ 3(3):225–331, 2009.  
情報検索のためのランキング学習。情報検索の基礎とトレンド、3(3):225–331、2009年。

[13] S. Rendle, C. Freudenthaler, Z. Gantner, and L. Schmidt-Thieme.  
BPR: Bayesian personalized ranking from implicit feedback. In Proceedings of UAI, pages 452–461, 2009.  
BPR：暗黙のフィードバックからのベイズ的パーソナライズドランキング。UAIの論文集、ページ452–461、2009年。

[14] S. S. Shapiro and M. B. Wilk.  
An analysis of variance test for normality (complete samples). Biometrika 52 (3-4), pages 591–611, 1965.  
正規性のための分散分析テスト（完全サンプル）。バイオメトリカ52 (3-4)、ページ591–611、1965年。

[15] R. W. White and D. Kelly.  
A study on the effects of personalization and task information on implicit feedback performance. In Proceedings of CIKM, pages 297–306, 2006.  
パーソナライズとタスク情報が暗黙のフィードバックパフォーマンスに与える影響に関する研究。CIKMの論文集、ページ297–306、2006年。

[16] J. Xu and H. Li.  
Adarank: A boosting algorithm for information retrieval. In Proceedings of SIGIR, pages 391–398, 2007.  
Adarank：情報検索のためのブースティングアルゴリズム。SIGIRの論文集、ページ391–398、2007年。

[17] S. Xu, H. Jiang, and F. C. M. Lau.  
Mining user dwell time for personalized web search re-ranking. In Proceedings of _IJCAI, pages 2367–2372, 2011._  
ユーザーの滞在時間をマイニングしてパーソナライズされたウェブ検索の再ランキングを行う。IJCAIの論文集、ページ2367–2372、2011年。

[18] S. Xu, Y. Zhu, H. Jiang, and F. C. M. Lau.  
A user-oriented webpage ranking algorithm based on user attention time. In _Proceedings of the 23rd National Conference on Artificial_ _Intelligence - Volume 2, pages 1255–1260, 2008._  
ユーザーの注意時間に基づくユーザー指向のウェブページランキングアルゴリズム。第23回人工知能全国会議の論文集 - ボリューム2、ページ1255–1260、2008年。

[19] P. Yin, P. Luo, W.-C. Lee, and M. Wang.  
Silence is also evidence: Interpreting dwell time for recommendation from psychological perspective. In Proceedings of SIGKDD, pages 989–997, New York, NY, USA, 2013. ACM.  
沈黙も証拠である：心理的視点からの推薦のための滞在時間の解釈。SIGKDDの論文集、ページ989–997、ニューヨーク、NY、USA、2013年。ACM。

[20] Z. Zheng, K. Chen, G. Sun, and H. Zha.  
A regression framework for learning ranking functions using relative relevance judgments. In Proceedings of SIGIR, pages 287–294, 2007.  
相対的関連判断を用いたランキング関数学習のための回帰フレームワーク。SIGIRの論文集、ページ287–294、2007年。

|Signal|MAP|NDCG|NDCG@10|  
|---|---|---|---|  
|Click as Target Dwell Time as Target|0.3773 0.3779|0.7439 0.7457|0.7434 0.7451|  
|Signal|MAP|NDCG|NDCG@10|  
|---|---|---|---|  
|Click as Target Dwell Time as Target|0.6275 0.6287|0.5820 0.5832|0.5813 0.5826|  
|Signal|MAP|NDCG|NDCG@10|  
|---|---|---|---|  
|Click as Target Dwell Time as Target|0.6275 0.6648|0.5578 0.5596|0.5570 0.5589|  
