...
NewsPicksはこの目的を達成する為に、Curation、Contents、Commentの3つの視点を用いたサービスを提供しています。

このミッションを踏まえると、必ずしも特定の一つのメディアモデルに当てはまるというよりは、4種類のメディアモデルの要素を含んだ上でやっと達成できるようなミッションなのかなと思います。
「新しい視点を集めて」みたいな観点は議論主義モデル的な要素だと思いますし、「経済の未来をひらく」だったり「ビジネスを楽しむ」ためには自由主義モデル的にユーザ自身の専門性を高められるような情報を提供していくことも重要だと思います。また、「誰もがビジネスを楽しめる」という観点では、参加主義モデル的に全ユーザに必要な共通認識的な情報を分かりやすい形で提供することも必要でしょう。
...
実際、数年前は「ワンフィード」と呼ばれるUIで、メイン画面に表示する記事を単一の推薦システムによって選択していました。当時自分はまだインターンで関わっていなかったのですが、推薦システムに求められる要件というか性質が混在していて、凄く難しい開発だったんじゃないかなと想像しています...!

しかし現在のNewsPicksは、「今日のニュース」や「話題をまとめよみ」、「金融・経済」「ビジネス」などの各カテゴリセクション、「あなたへのおすすめ」など、複数のセクションが共存するようなUIを採用しています。個人的にはこのUIは、各セクションが異なる役割を担ってくれているような印象を持っていて、NewsPicksのミッションを達成するためには非常に重要なんじゃないかなと思っています。
自分の認識では、「今日のニュース」や「話題をまとめよみ」は議論主義モデルの要素、各カテゴリセクションは参加主義モデルの要素、そして「あなたへのおすすめ」は自由主義モデルの要素を担っているように思います。
(もちろんNewsPicksの提供してるサービスの軸は配信するニュースを選ぶCurationだけでなく、オリジナル記事やオリジナル動画などのContents、ユーザからのCommentもミッションの達成の為に重要です!)

というわけで、「ワンフィード」を経て複数のセクションが共存するようなUIに至り、推薦システムに求められる性質を役割分担できたことで、より推薦システムの開発・改善の方向付けが明確になったのかなと感じています:)
ちなみに自分は現在、各カテゴリセクションや「あなたへのおすすめ」の推薦システムの開発・運用・改善に関わっているので、それぞれ参加主義モデルと自由主義モデルで求められる性質を頭に留めつつ、システム改善に取り組んでみようかなと思っています...!
...
論文では、4種類のメディアモデルが推薦システムに期待する性質から導出した、ニュース推薦システムの評価のための5つの多様性指標群 RADio (Rank-Aware Divergence metrics, ioはよく分からず...!:thinking:) を提案しています。
