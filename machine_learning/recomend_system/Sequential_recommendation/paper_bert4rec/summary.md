...
- Sequential推薦の既存研究における、手法の発展の流れは以下の様な感じ:
...
- それに対して本論文では、以下の２つの制限から、**uni-directionalなモデルは逐次推薦タスクに適したuser preference表現を学習するには不十分**であると主張。
  - 制限1: uni-directionalなモデルでは、各item(=user-item interaction)がそれよりも過去のitems(=interactions)からの情報しか考慮できないため、hidden representationの力が制限される。
...
- よって本論文では、uni-directionalではなくbi-directionalなモデル(要はBERTのような bi-directional self-attention model)を採用した手法 **BERT4Rec**を提案し、Sequential推薦タスクにより適したuser preference 表現の学習を目指す。
