...
### 2.1.1. Heterogeneous Embedding Module.
...
このモジュールでは、bert-basedのモデルで一般的に行われるように、embedding lookup(?)を通して生の入力を分散表現(=embedding?)に変換する。

The original input is a heterogeneous interaction sequence including purchase actions and non-purchase actions.
元の入力は、購入行動と非購入行動を含む異種interaction sequenceである。
...
埋め込みモジュールに生の入力を入力する代わりに、各相互作用 $i$ を特徴量 $f_{i}^{j}$ の集合で表現することにする($j \in 1, 2, \cdots, |J|$)、ここで、$|J|$は各相互作用の特徴量の総数を表す。
...
sequential encoding moduleの出力は、入力シーケンス(=$\mathbf{H}$ ??)と同じ長さの隠れベクトルのシーケンスである。
...
先行研究[9]は、最後(=sequenceの最後のtoken?:thinking:)の隠れベクトルと項目埋め込みとの内積を計算し、対応する項目のスコアを生成する。(=シンプルにdot-productによるscore prediction module:thinking:)
...
我々は、task-awareな新しいattentional readout modulを提案する。このモジュールは、タスクに特化した表現を生成するために、異なるアイテム(各タスクのラベル)と異なるタスクが、attentionメカニズムによって、隠れsequenceの異なる部分sequenceに注目することを可能にする。
...
具体的には、secuential encoding moduleの出力の $i$ 番目の埋め込みを$\mathbf{h}_{i}$ とし、あるタスクの特定のitem $w$ に対する埋め込みを $\mathbf{e}_{w}$ とすると、attentional readout操作は次のように記述できる:
...
バートエンコーダーは3つのトランスフォーマー層で構成され、ヘッド数は4である。(うんうん、推薦用のarchitectureは小さくて良いんだよね!:thinking:)
