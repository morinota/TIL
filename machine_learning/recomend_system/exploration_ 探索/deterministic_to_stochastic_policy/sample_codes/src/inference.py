...
def model_fn(model_dir: str):
    ...
    return Recommender(
        user_vectors=load_vectors_parquet(model_dir_path / "user_vectors", "user_id"),
        movie_vectors=load_vectors_parquet(
            model_dir_path / "movie_vectors", "movie_id"
        ),
    )
...
def predict_fn(
    ...
    return input_data.user_id, model.predict(input_data.user_id, input_data.k)
