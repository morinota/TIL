...
### 方法1 エントロピー正則化 (学習時の最適化関数にエントロピー正則化項を追加する)

- 学習時の最適化関数に、方策の行動確率分布 $\pi_{\theta}$ のエントロピーを高くなるように強制する、エントロピー正則化項を追加する。
  - 具体的には、式(2)で定義される学習目的関数 $J(\theta)$ に、エントロピー正則化項を追加する。
...
### 方法2　reward shaping(即時報酬関数 $r$ の定義を修正する)
...
### 方法3: Actionable Representation for Exploration (探索を表す要素をユーザ表現に追加する)

- 多くのMLアルゴリズムでは、効果的な表現を学習することが重要。RLも例外ではない。
- 前述のREINFORCEエージェントは、ユーザの過去の行動履歴 $H_{t}$ を入力として環境(ユーザ)の表現 $u_{s_{t}}$ を得る。
  - 数式で表すと $u_{s_t} = RNN_{\theta}(H_t)$
  - (うんうん、ユーザ状態を表す埋め込み表現を作ってるんだよね :thinking:)
  - ちなみに...
    - (RLにおけるagent = 方策に従って行動し、報酬を得る存在。agentは方策を含む幅広い概念と言って良い :thinking:)
- 方法のモチベーション:
  - もしユーザが（agentにとって）surprisingなアイテムと接触して高い報酬を与えた場合、**ユーザ状態 $u_{s_t}$ はその新しく発見されたユーザ興味の情報を取り込むように更新されるべき**!
  - これは、agentがsurprisingなフィードバックを考慮して、新しく発見されたユーザ興味に従って推薦を行えるようにするため。
- 方法の具体案:
  - **行動履歴 $H_t$ を拡張し、ユーザがinteractするアイテムがsurprisingかどうか、relevance(関連性)があるかどうかを示す特徴量を追加する**。
    - ($H_t$ の中の各tuppleに、surprisingか否か、relevantか否かを示すフラグを追加するってこと??:thinking:)
    - (あ、実際には、relevanceか否かの情報は即時の外発的報酬関数 $r$ で得られるから、surprisingかを示す特徴量を1つだけ追加してるっぽい!:thinking:)
  - 拡張した行動履歴: $H_t = \{(A_0, a_0, r_0, i_0), \cdots, (A_{t-1}, a_{t-1}, r_{t-1}, i_{t-1})\}$
  - ここで、追加したbinaryの特徴量 $i_{t'}$ について
    - $i_{t'} = 1$ は**interactしたアイテムがsurprisingかつrelevant(=つまり、エージェントが知らなかった新しいユーザ興味...!:thinking:)であることを示す**。 そうでない場合は $i_{t'} = 0$ となる。
    - $i_{t'} = 1$ になるための具体的な2つの条件（以下の両方を満たす必要がある）
      - 1. 推薦アイテム $a_{t'}$ の属性 (ex. トピッククラスタ、アイテムカテゴリなど) が　時刻 $t'$ までにユーザが相互作用したアイテム集合 $I_{t'}$ と異なる (i.e. surprisingである条件!:thinking:)
      - 2. $r_{t'} > 0$ である (i.e. relevantである条件!:thinking:)
    - 追加された特徴量 $i_{t'}$ は、アイテム $a_{t'}$ を記述する他の特徴量と一緒にRNNに渡され ユーザ状態 $u_{s_t}$ にencodeされる。
      - (あ、他の特徴量 (トピッククラスタとか!) も一緒にユーザ状態にencodeされるのか...!:thinking:)
