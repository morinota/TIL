...
- 論文の中身
  - カルーセル推薦を文脈つき多腕バンディット問題として定式化
  - **カスケードモデルを組み合わせる**（要するに「ユーザは手前側に表示されたアイテムから見ていくはず...!」みたいなユーザ行動の仮定を使うってこと!）
  - ユーザのクラスタリング or 文脈ベクトルを使った推薦手法の比較
  - Deezerのデータを用いたオフライン評価 & A/Bテストでのオンライン評価
- 実験コードのlink: https://github.com/deezer/carousel_bandits
...
シミュレーションなので、乱数を生成するために、擬似的に真の値を用意しておく。(この場合は、各(ユーザ, プレイリスト)ペアの、ストリーミングするか否かの確率分布の期待値を用意しておく)
