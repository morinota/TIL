...
さて、DP で解ける問題は最適化問題だけではない。
ナップサック問題とよく似た問題として、次の部分和問題も有名らしい。
むしろ、この部分和問題の方が簡単な問題らしい。
...
- ナップサック問題とほぼ同じ。
- dp[i+1][j] :=i番目までの整数の中からいくつか選んで総和をjとする事が可能かどうか(bool値)
- dp[i][j](j = 0,1,...,A)を使って、dp[i+1][j]の値(j = 0,1,...,A)を更新する事を考える。

- dp[i+1][j]の値を求めるには以下のようになる。
  - 整数a[i]を選ぶ場合
    - dp[i]i-a[i]]がTrueなら、dp[i+1][j]もTrue
  - 整数a[i]を選ばない場合
    - dp[i][j]がTrueなら、dp[i+1][j]もTrue
...
dp[i+1][j] = \begin{cases}
  dp[i][i-a[i]] \quad | \quad dp[i][j] & (j \geq a[i])
  \\
  dp[i][j] & (j < a[i])
\end{cases}
...
(0個の整数の和は0とみなせるので、dp[0][0]のみTrue)
