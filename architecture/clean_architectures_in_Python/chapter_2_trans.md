...
うまく設計されたシステムの主な目標のひとつは、コントロールを達成することだ。
...
AIに対する立場がどうであれ、責任の所在が明確であれば、システムはよりよく機能するという点では、誰もが同意するところだろう。
...
ソフトウェアであれ人間社会であれ、あるコンポーネントができること、あるいはすべきことが不明確であることは常に危険である。
...
これは、政治的な概念（divide et impera）の延長線上にあるもので、複雑なひとつのシステムを支配するよりも、相互に結びついた小さなシステムの集合を支配するほうが単純だというものだ。
...
マシンについて議論するときは常に、マシンが機能する方法（ランタイム）と、マシンが構築された、あるいは今後修正される方法（開発タイム）の両方を考慮しなければならない。
In principle, computers don't care where data comes from and where it goes.
原則的に、コンピューターはデータがどこから来てどこへ行くかを気にしない。
...
データ型やフォーマットに関する知識は、まさにカップリングの一種である。
...
前章では、システムには、システムの中核をなす重要なコンポーネント（ユースケース）と、それほど中心的ではなく、しばしば実装の詳細とみなされるコンポーネントがあることを発見した。
...
繰り返しになるが、これらを「ディテール」と呼ぶことは、重要でないとか、実装が些細であるということを意味するのではなく、異なる実装に置き換えてもシステムの中核（ビジネス・ロジック）には影響を与えないということを念頭に置いてほしい。
...
いずれにせよ、2つの正反対の依存関係があることは間違いなく混乱を招くし、ショップのような "周辺 "コンポーネントを置き換えることも難しくなる。
...
クリーンなアーキテクチャは、コンポーネントの概念的な階層と、レイヤーアプローチによる型の階層の両方を捉えようとしている。
...
クリーンなアーキテクチャでは、システムのコンポーネントは分類され、特定のレイヤーに属し、同じレイヤーまたは異なるレイヤーに属するコンポーネント間の通信に関するルールがある。
...
特に、クリーン・アーキテクチャは球状の構造であり、内側（下層）の層は外側（上層）の層に完全に包含され、前者は後者の存在に気づかない。
...
コンピュータ・サイエンスでは、「より低い」「より高い」という言葉は、ほとんどの場合、抽象化のレベルを指すのであって、システムにとってのコンポーネントの重要性を指すのではないことを覚えておいてほしい。
Each part of a system is important, otherwise it would not be there.
システムの各部分は重要であり、そうでなければ存在しない。
...
図に描かれている主なレイヤーを見てみよう。特定の実装では、新しいレイヤーを作成したり、これらのレイヤーのいくつかを複数のレイヤーに分割したりする必要があるかもしれないことに留意してほしい。
...
しかし、プロジェクトが中世の写本を分析するためのツールであれば、文とその特徴を分離する必要があるかもしれない。
...
エンティティは同じレイヤーに住んでいるため相互知識があり、アーキテクチャによって直接相互作用することができる。
...
これは、ある実体を表すPythonクラスの1つが、別のクラスを直接使用し、インスタンス化してそのメソッドを呼び出すことができることを意味する。
...
データベースを呼び出したり、プレゼンテーション・フレームワークが提供するメソッドにアクセスしたり、ユースケースをインスタンス化したりすることはできない。
...
エンティティ・レイヤーは、外側のレイヤーがデータ交換に使用できる型の強固な基盤を提供するもので、ビジネスの語彙と考えることができる。
...
先に述べたように、クリーンなシステムの最も重要な部分はユースケースである。ユースケースはビジネスルールを実装するものであり、システム自体の存在理由の中核をなすものだからである。
Use cases are the processes that happen in your application, where you use your domain models to work on real data.
ユースケースとは、アプリケーションの中で発生するプロセスのことで、ドメインモデルを使って実際のデータを操作する。
...
例としては、ユーザーがログインする、特定のフィルタを使った検索が実行される、またはユーザーがカートの内容を購入したいときに銀行取引が発生する、などがあります。

Use cases should be as small as possible.
ユースケースはできるだけ小さくすべきである。
...
典型的な例はデータ・ストレージで、内部的な詳細は実装によって大きく異なることがある。
...
先ほどの簡単な例を思い出すと、ここがデータベース・インターフェースの場所だ。
...
ゲートウェイはエンティティにアクセスできるので、インターフェースは、ユースケースに自由にアクセスできるように、そのレイヤーで定義された型のオブジェクトを自由に受け取り、返すことができる。
...
しかし、ゲートウェイは外部システムの実装をマスクするために使用されるため、ゲートウェイがユースケースを呼び出すことは稀である。
...
ゲートウェイ層は外部システム層と密接に関係しているため、両者は破線で区切られている。
...
アーキテクチャのこの部分には、前のレイヤーで定義されたインターフェースを実装するコンポーネントが配置される。
...
システムがそのインターフェイスの複数の実装を同時にサポートしたいかもしれないので、同じインターフェイスが1つ以上の具象コンポーネントによって実装されるかもしれない。
...
例えば、あるユースケースをHTTP APIとコマンドライン・インターフェイスの両方で公開したいとか、ある設定値に応じて異なるタイプのストレージのサポートを提供したいとか。
...
外部」という形容詞は、必ずしもそのシステムが他者によって開発されたものであったり、ウェブ・フレームワークやデータベースのような複雑なシステムであることを意味するものではないことを覚えておいてほしい。
...
この言葉にはトポロジー的な意味があり、私たちが話しているシステムがアーキテクチャの中核から外れた周辺的なものであること、つまりビジネス・ロジックを実装していないことを示している。
...
特定のシステムを包むために作られるゲートウェイとの関係を理解するのは簡単だが、外部システムがユースケースやエンティティに対して何をすべきかは、あまり明確ではないかもしれない。
...
ユースケースに関しては、外部システムは通常、ユーザーがビジネス・ロジックを実行するためのトリガーとなるシステム部分である。
...
ユーザーがボタンをクリックしたり、URLを訪問したり、コマンドを実行したりすることは、ユースケースを直接実行する外部システムとのインタラクションの典型的な例である。
...
ユースケースに使われる外部システムと、ユースケースを呼び出したい外部システムの違いを指摘したい。
...
最初のケースでは、通信の方向は外側であり、クリーンなアーキテクチャでは、インターフェイスなしでは外側に行けないことがわかっている。
Thus, when we access an external system from a use case we always need an interface.
したがって、ユースケースから外部システムにアクセスする際には、常にインターフェースが必要になる。
...
代わりに外部システムがユースケースを呼び出したい場合、通信の方向は内側になり、外部レイヤーは内部レイヤーに完全にアクセスできるため、これは直接許可される。

This, practically speaking, translates into two extreme cases, well represented by a database and a web framework.
これは現実的に言えば、データベースとウェブ・フレームワークに代表される2つの極端なケースになる。
...
ユースケースがストレージシステムにアクセスする場合、両者の間には緩やかな結合が必要である。これが、ストレージをインターフェースでラップし、ユースケースでそれを想定する理由である。
...
このアーキテクチャでは、レイヤーが深ければ深いほど、コンテンツはより抽象的になる。
...
同じレイヤーに住むエレメント同士の通信は自由だが、他のレイヤーに割り当てられたエレメントと通信したい場合は、ある簡単なルールに従わなければならない。
...
このルールは、クリーン・アーキテクチャにおいて最も重要なものであり、クリーン・アーキテクチャの核となる表現そのものかもしれない。

### The Golden Rule: talk inwards with simple structures, talk outwards through interfaces. シンプルな構造で内向きに話し、インターフェースで外向きに話す。
...
つまり、基本的な構造、つまり、使用しているプログラミング言語が提供するエンティティやあらゆるものを使って、より抽象的な要素にデータを渡すのだ。
...
つまり、特定の実装を参照することなく、コンポーネントの期待されるAPIのみを使用することです。
...
アウターレイヤーが作られると、そこに住むエレメントはそれらのインターフェイスに自分自身をプラグインし、実用的な実装を提供することになる。
...
各レイヤーは、内部レイヤーに住むエレメントからAPIによってアクセスすることができる。APIとは、エントリーポイント（メソッドまたはオブジェクト）の固定されたコレクション[1]である。
...
例として、あるユースケースがインターフェイスを使ってストレージ・レイヤーにアクセスしているが、これが遅すぎることが判明したとしよう。
...
その場合、使用している特定のデータベースのAPIに直接アクセスすることになるが、内部レイヤー（ユースケース）が外部レイヤー（外部インターフェース）にアクセスすることになり、データフローが壊れてしまう。
...
将来、誰かがあなたが使っている特定のデータベースを別のものに置き換えたくなった場合、新しいデータベースはおそらく同じデータで同じAPIエントリーポイントを提供しないので、このことを認識しておく必要がある。
