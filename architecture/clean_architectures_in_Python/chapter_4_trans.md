...
HTTPエンドポイントを公開するには、Pythonで書かれたウェブ・サーバーが必要で、今回はFlaskを選んだ。
...
Flaskの設定パラメータについて詳しくは、このページを読んでください。
...
エンドポイントとは、最終的にはユーザーが特定のURLにリクエストを送ったときに実行される関数のことで、最終的なゴールは特定の結果を生み出すコードを持つことなので、TDDで作業することはできる。
...
インストールすると、クライアントのようないくつかのフィクスチャが自動的に利用可能になるので、それらをインポートする必要はありません。
Moreover, it tries to access another fixture named app that you have to define.
さらに、あなたが定義しなければならないappという別のフィクスチャにアクセスしようとします。
...
フィクスチャはテストファイルで直接定義することができますが、フィクスチャをグローバルに利用できるようにするには、pytestによって自動的にロードされるconftest.pyファイルが最適です。
...
関数appは、フラグTESTINGをTrueに設定する設定testingを使用して、Flaskアプリを作成するためにアプリファクトリーを実行します。
...
最初のパートは、いくつかのインポートを含み、辞書から部屋を設定する。
...
最初の行はモック・ユースケースを初期化し、前回作成した固定ルーム変数を返すように指示している。
...
テストの中心部分は、APIエンドポイントを取得する行で、HTTP GETリクエストを送信し、サーバーのレスポンスを収集する。
...
Flaskがエンドポイントを登録する方法であるブループリントの設定とは別に、エンドポイントを実行するシンプルな関数を作成し、GETリクエストに対応するenpoint /roomsを割り当てて装飾します。
...
この関数はいくつかのロジックを実行し、最終的にJSONデータ、正しいMIMEタイプ、ロジックの成否を表すHTTPステータスを含むResponseを返す。
...
これは、コマンドライン・インターフェイスで使ったコードとまったく同じものだ。
...
コードの最後の部分は、RoomJsonEncoderを使用してユースケースの結果をシリアライズし、HTTPステータスを200（成功）に設定し、適切なHTTPレスポンスを作成します。

This shows you the power of the clean architecture in a nutshell.
クリーン・アーキテクチャの威力を端的に示している。
...
CLIインターフェイスやウェブサービスを書くことは、プレゼンテーションレイヤーにおいてのみ異なる。
...
PythonのWebアプリケーションは、Web Server Gateway InterfaceまたはWSGIと呼ばれる共通のインターフェースを公開しています。
...
Flaskの開発用Webサーバを動かすには、プロジェクトのメインフォルダ、つまりcli.pyと同じディレクトリにwsgi.pyファイルを定義する必要があります。
...
関数create_appはファクトリーなので、これを実行するだけでよい。
...
この時点で、このファイルを含むディレクトリでFLASK_CONFIG="development" flask runを実行すると、以下のような素敵なメッセージが表示されるはずだ。
...
私たちが作り上げたレイヤード・アーキテクチャーのパワーを理解していただけただろうか。
...
モデルのリストを "ただ "プリントアウトするために多くのコードを書いたのは確かだが、私たちが書いたコードは、簡単に拡張したり修正したりできるスケルトンである。
...
入力を必要とせず、エラー条件を返すこともできないので、私たちが書いたコードは入力検証とエラー管理を完全に無視していた。
