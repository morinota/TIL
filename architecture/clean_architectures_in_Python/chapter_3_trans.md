...
Rent-o-Matic "プロジェクトの目標は、部屋を貸す会社のためのシンプルな検索エンジンを作ることだ。
...
建築上の問題とその解決方法に焦点を当てるため、説明はあえて最小限にとどめている。
...
クリーン・アーキテクチャ・モデルによって推し進められるように、我々はシステムの異なるレイヤーを分離することに興味がある。
...
クリーン・アーキテクチャのコンセプトを実装する方法は複数あり、あなたが思いつくコードは、選択した言語で何ができるかに強く依存することを覚えておいてほしい。
...
以下は、Pythonによるクリーンなアーキテクチャの例であり、これから紹介するモデル、ユースケース、その他のコンポーネントの実装は、可能なソリューションの一つに過ぎない。
...
テストを書くたびに、テスト・スイートを実行し、エラー（またはそれ以上）が出るかどうかをチェックする必要があります。そして、私が解決策として与えるコードは、テスト・スイートをパスさせるはずです。
...
データクラスを使うのは、このような単純なモデルを実装するコンパクトな方法だからだが、標準クラスを使ったり、メソッド**init**を明示的に実装したりするのは自由だ。
...
このモデルを初期化するためのデータを他のレイヤーから受け取ること、そしてこのデータが辞書である可能性が高いことを考えると、この種の構造からモデルを初期化できるメソッドを作成することは有用である。
...
前述したのと同じ理由で、モデルを辞書に変換できると便利です。そうすれば、JSONや同様の言語に依存しないフォーマットに簡単にシリアライズできます。
...
繰り返しになるが、データクラスは**eq**の実装をすぐに提供してくれるので、これは非常に簡単である。
...
典型的なシリアライズ形式はJSONで、これはウェブベースのAPIで広く受け入れられている標準だからだ。
...
アプリケーション内部で実行される実際のビジネス・ロジックを実装する時が来た。
...
最も単純なユースケースは、リポジトリに保存されているすべての部屋をフェッチして返すものです。
...
リポジトリは我々のストレージ・コンポーネントであり、クリーン・アーキテクチャによれば、それは外部レベル（外部システム）に実装される。
...
テストの観点からは、インターフェイスにアクセスするコードを実行する最善の方法は、後者をモックすることである。
Put this code in the file tests/use_cases/test_room_list.py
このコードをtests/use_cases/test_room_list.pyに記述します。
...
まず、リポジトリがメソッドリストを提供し、テストの上で作成したモデルのリストを返すようにモックします。
...
リポジトリのメソッドリストを呼び出すことは、ユースケースが実行することになっている発信クエリのアクションであり、ユニットテストのルールによれば、発信クエリをテストすべきではありません。
...
まず第一に、このユースケースはリポジトリの特定の機能のラッパーにすぎず、エラーチェックを含んでいません。
...
ユースケースはビジネス・ロジック、つまりプロセスを表している。つまり、プログラミング言語でできる最も単純な実装は関数であり、入力引数を受け取って出力データを返すコードである。
...
だから、他の多くのケースと同じように、関数を使うべきかクラスを使うべきかが問題になるのだが、私の答えは、実装するアルゴリズムの複雑さの度合いによるというものだ。
...
しかし、よりシンプルなソリューション（関数）で同じことができるのに、クラスを使わないように注意してほしい。
Remember that code has to be maintained, so the simpler it is, the better.
コードはメンテナンスされなければならないので、シンプルであればあるほど良いということを覚えておいてほしい。
...
このオブジェクトは一般的に「リポジトリ」という愛称で呼ばれ、ユースケースの情報源となる。
...
クリーンアーキテクチャのリポジトリが提供する抽象化レベルは、フレームワークの ORM や SQLAlchemy のようなツールが提供する抽象化レベルよりも高いことは注目に値する。
...
リポジトリは、アプリケーションが必要とするエンドポイントのみを提供し、アプリケーションが実装する特定のビジネス問題に合わせたインターフェースを持つ。
...
具体的な技術の観点から問題を明確にすると、SQLAlchemyはSQLデータベースへのアクセスを抽象化する素晴らしいツールです。したがって、リポジトリの内部実装は、たとえばPostgreSQLデータベースにアクセスするためにSQLAlchemyを使うことができます。
...
APIは、ユースケースがデータを取得するために呼び出す関数の縮小されたセットであり、内部実装は、生のSQLクエリからRabbitMQネットワークを介したリモートコールの複雑なシステムまで、同じ目標を達成するために幅広いソリューションを使用することができます。
...
リポジトリの非常に重要な特徴は、ドメインモデルを返すことができることであり、これはフレームワークORMが通常行うことと一致している。
...
第3レイヤーの要素は、内部レイヤーで定義されたすべての要素にアクセスできる。つまり、ドメインモデルとユースケースは、リポジトリから直接呼び出して使用できる。
...
この場合、メソッド・リストの振る舞いをチェックする単一のテストが必要だ。
...
このクラスが実際のデータベースやその他のストレージ・タイプのラッパーになることは容易に想像できる。
...
これまでのところ、ドメインモデル、シリアライザー、ユースケース、リポジトリを作成したが、すべてを統合するシステムがまだ欠けている。
...
このシステムは、ユーザーから呼び出しパラメータを取得し、リポジトリでユースケースを初期化し、リポジトリからドメインモデルをフェッチするユースケースを実行し、ユーザーにそれらを返さなければならない。
...
この場合、ユーザーにシステムを照会し、ストレージシステムに含まれる部屋のリストを取得するためのインターフェイスを与えたいのだが、最も単純な選択肢はコマンドラインツールである。
...
これは一般的に、クリーンなアーキテクチャと、それに接続する外部システムをどのように使うかということだ。
...
この章で見たのは、クリーン・アーキテクチャーの核心部分である。
...
私たちは、エンティティ（Roomクラス）、ユースケース（room_list_use_case関数）、ゲートウェイ、外部システム（MemRepoクラス）の標準的なレイヤーを探索し、レイヤーに分離することの利点を理解し始めた。
...
そのため、本書の残りの部分を、より複雑なケースに対応するための強化方法を紹介することに割く。
...
第4章ではウェブインターフェースについて、第5章ではより豊富なクエリー言語とエラー管理について、第6章、第7章、第8章ではデータベースのような実際の外部システムとの統合について述べる。
