...
前章では、「バッチ数量が変更されました」というイベントをどのように受け取るか、また、再割り当てをどのように外部に通知するかについて、実際に話をしたことはなかった。
...
この章では、イベントメタファーを拡張して、システムからのメッセージの入出力をどのように処理するかを紹介したいと思います。
Internally, the core of our application is now a message processor.
内部的には、アプリケーションの中核はメッセージプロセッサになっています。
Let’s follow through on that so it becomes a message processor externally as well.
続いて、外部でもメッセージプロセッサーになるようにしましょう。
As shown in Figure 11-1, our application will receive events from external sources via an external message bus (we’ll use Redis pub
図 11-1 に示すように、このアプリケーションは外部ソースから外部メッセージバス経由でイベントを受け取ります (ここでは Redis pub

![](https://learning.oreilly.com/api/v2/epubs/urn:orm:book:9781492052197/files/assets/apwp_1101.png)

- TIP ヒント

- The code for this chapter is in the chapter_11_external_events branch on GitHub: この章のコードは、GitHubのchapter_11_external_eventsブランチにあります。
...
したがって、システムを単純に分割しようとすると、図11-2のようになります（Allocationではなく、Batchesという名詞の名前をシステムにつけていることに注意してください）。
...
これらの各ステップは、システム内のコマンドとして考えることができます。`ReserveStock`, `ConfirmReservation`, `DispatchGoods`, `MakeCustomerVIP` などです。
...
また、在庫モデルを更新する必要があり、お客様の注文を再配置する必要があるかもしれません。
...
これも一応うまくいくのですが、今度は依存関係のグラフがぐちゃぐちゃになってしまいました。
...
図11-5に示すように、ユーザーが`MISBEGOTTEN-RUG`を3つ注文した直後に、ネットワークエラーが発生したとしましょう。
...
この故障のカスケードは、一種の時間的結合と考えることができます。システムのどの部分が動作するにも、同時に動作する必要があるのです。
...
- We’re using the term coupling here, but there’s another way to describe the relationships between our systems. Connascence is a term used by some authors to describe the different types of coupling. ここではカップリングという言葉を使っていますが、システム間の関係を表現する別の方法があります。 Connascenceは、異なるタイプのカップリングを表現するために一部の著者が使用している用語です。
...
- In our first example of a distributed ball of mud, we see Connascence of Execution: multiple components need to know the correct order of work for an operation to be successful. 最初の泥の塊の例では、「実行の連鎖」が見られる。作業を成功させるためには、複数のコンポーネントが正しい作業順序を知る必要がある。

- When thinking about error conditions here, we’re talking about Connascence of Timing: multiple things have to happen, one after another, for the operation to work. ここでいうエラーとは、「Connascence of Timing」のことで、複数のことが次々に起こらないと動作しないことを指します。

- When we replace our RPC-style system with events, we replace both of these types of connascence with a weaker type. That’s Connascence of Name: multiple components need to agree only on the name of an event and the names of fields it carries. RPCスタイルのシステムをイベントに置き換えるとき、この2つのタイプの接続をより弱いタイプに置き換えます。 つまり、「名前の接続」です。複数のコンポーネントは、イベントの名前とそれが運ぶフィールドの名前についてだけ合意すればよいのです。

- We can never completely avoid coupling, except by having our software not talk to any other software. What we want is to avoid inappropriate coupling. Connascence provides a mental model for understanding the strength and type of coupling inherent in different architectural styles. Read all about it at connascence.io. ソフトウェアが他のソフトウェアと会話しないようにすることを除けば、結合を完全に避けることはできない。 私たちが望むのは、不適切な結合を避けることです。 Connascenceは、異なるアーキテクチャスタイルに内在するカップリングの強さとタイプを理解するためのメンタルモデルを提供します。 connascence.ioでそのすべてを読むことができます。
...
このように物事を分けて考えると、どのシステムが何に責任を持つべきかが、少しわかりやすくなります。
When thinking about ordering, really we want to make sure that when we place an order, the order is placed.
注文について考えるとき、私たちが本当にしたいのは、注文をしたときに、その注文が行われるようにすることです。
Everything else can happen later, so long as it happens.
それ以外のことは後回しでいいのです。

- NOTE 注

- If this sounds familiar, it should! Segregating responsibilities is the same process we went through when designing our aggregates and commands. もしこれが聞き覚えのあるものであれば、その通りです。 責任の分担は、アグリゲートとコマンドを設計するときに行ったのと同じプロセスです。

Like aggregates, microservices should be consistency boundaries.
アグリゲートのように、マイクロサービスも一貫性の境界であるべきです。
...
各サービスは外界からのコマンドを受け付け、その結果を記録するためにイベントを発生させる。
Other services can listen to those events to trigger the next steps in the workflow.
他のサービスは、それらのイベントをリッスンして、ワークフローの次のステップをトリガーすることができる。
...
Distributed Ball of Mud のアンチパターンを避けるために、時間的に結合された HTTP API コールの代わりに、非同期メッセージングを使用してシステムを統合したいと思います。
...
BatchQuantityChanged` メッセージが上流のシステムから外部メッセージとして送られてくるようにし、下流のシステムが listen できるように `Allocated` イベントを発行するようにしたいと思います。
...
メッセージバスのように、あるシステムから別のシステムへ、しかしサービスのためにイベントを取得する方法が必要です。
This piece of infrastructure is often called a message broker.
このようなインフラは、しばしばメッセージブローカーと呼ばれます。
The role of a message broker is to take messages from publishers and deliver them to subscribers.
メッセージブローカーの役割は、パブリッシャーからメッセージを受け取り、それをサブスクライバーに配信することです。

At MADE.com, we use Event Store; Kafka or RabbitMQ are valid alternatives.
MADE.comでは、Event Storeを使用しています。KafkaやRabbitMQは有効な代替手段です。
A lightweight solution based on Redis pub
Redisパブをベースとした軽量なソリューション

- NOTE 注

- We’re glossing over the complexity involved in choosing the right messaging platform. Concerns like message ordering, failure handling, and idempotency all need to be thought through. For a few pointers, see “Footguns”. 正しいメッセージング・プラットフォームの選択には、複雑さがつきまといます。 メッセージの順序、障害処理、およびべき等といった懸念事項は、すべて考え抜かれる必要があります。 いくつかのポインタについては、"Footguns "を参照してください。

Our new flow will look like Figure 11-6:
新しいフローは図 11-6 のようになります。
...
Redis は全体のプロセスを開始する `BatchQuantityChanged` イベントを提供し、割り当てられたイベントは最後に再び Redis にパブリッシュされます。
